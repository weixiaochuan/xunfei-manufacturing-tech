...
/// 创建任务执行会话
#[tauri::command]
pub fn create_task_session(db: State<'_, Database>, plan_path: String) -> Result<TaskSession, String> {
...
pub fn start_session_phase(
    ...
    SessionManagerService::start_phase(&db, &session_id, phase_index)
        .map_err(|e| e.to_string())
}
...
pub fn skip_session_phase(
    ...
    SessionManagerService::skip_phase(&db, &session_id, phase_index)
        .map_err(|e| e.to_string())
}
...
pub fn retry_session_phase(
    ...
    SessionManagerService::retry_phase(&db, &session_id, phase_index)
        .map_err(|e| e.to_string())
}
...
/// 添加执行日志
#[tauri::command]
pub fn add_execution_log(db: State<'_, Database>, log: CreateExecutionLogInput) -> Result<(), String> {
...
pub fn get_execution_logs(
    ...
    match phase_id {
        ...
        None => db.get_logs_by_session(&session_id).map_err(|e| e.to_string()),
    }
}
...
/// 列出已打开的项目会话
#[tauri::command]
pub fn list_open_project_sessions(
    db: State<'_, Database>,
