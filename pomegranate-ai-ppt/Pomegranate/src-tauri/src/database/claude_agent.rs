...
impl Database {
    ...
    pub fn list_agent_events(
        &self,
        session_id: &str,
    ) -> Result<Vec<ClaudeAgentEvent>, AppError> {
        let conn = self.conn.lock().map_err(|e| AppError::Custom(e.to_string()))?;
...
mod tests {
    ...
    fn update_status_flow() {
        ...
        db.update_agent_session_status(&s.id, "running", Some(42), None).unwrap();
        let s = db.get_agent_session(&s.id).unwrap();
        assert_eq!(s.status, "running");
        assert_eq!(s.exit_code, Some(42));

        db.update_agent_session_status(&s.id, "completed", Some(0), None).unwrap();
        let s = db.get_agent_session(&s.id).unwrap();
