...
export type PlanningSessionKind = "ai" | "agent";

export interface PlanningFileView {
  name: string;
  content: string;
...
export interface Note {
  ...
  /** 原始文件类型："pdf" / "docx" / "doc" / "xlsx" / "xls" / "xlsm" / "xlsb" / "ods" / null */
...
/** 当前进程实际使用的数据目录，直接来自 Rust AppState */
export interface RuntimeDataDirectory {
  path: string;
  source: string;
  exists: boolean;
  databasePath: string;
...
export interface AsrConfig {
  ...
  credentialMigrationStatus?: string | null;
...
export type CourseGraphAiRelationStatus = "pending" | "accepted" | "rejected";

export interface CourseGraphAiRelation {
  id: number;
  sourceNodeId: string;
...
export interface CourseGraphAiAnalysis {
  nodeId: string;
  nodeName: string;
  sourceKind: string;
  sourceRevision: string;
  ...
  modelId: number;
  relations: CourseGraphAiRelation[];
  createdAt: string;
  updatedAt: string;
}
...
export interface ResearchPaperSearchInput {
  query: string;
  limit?: number;
}
...
export type ResearchPaperKnowledgeDecision =
  | "recommended"
  | "consider"
  | "not_recommended";

export interface ResearchPaperKnowledgeRecommendationInput {
  query: string;
  paper: ResearchPaper;
}

export interface ResearchPaperKnowledgeRecommendation {
  decision: ResearchPaperKnowledgeDecision;
  reason: string;
  confidence: number;
  suggestedTags: string[];
  evaluationMode: "ai" | "local_fallback";
  warning?: string | null;
  modelId: number;
}
...
export interface ResearchPaperSourceStatus {
  name: string;
  available: boolean;
  resultCount: number;
  message?: string | null;
...
export interface ResearchPaperSearchResult {
  ...
  source: string;
  sources: ResearchPaperSourceStatus[];
  warnings: string[];
}
...
export interface ResearchKeywordOverlap {
  keyword: string;
  paperIds: string[];
  analysis: string;
}
...
export interface ResearchProjectRecommendation {
  title: string;
  action: string;
  rationale: string;
...
export interface ResearchAnalysisResult {
  ...
  papers: ResearchPaperAnalysis[];
  keywordOverlaps: ResearchKeywordOverlap[];
  comparisons: ResearchComparison[];
  recommendations: ResearchProjectRecommendation[];
  graphNodes: ResearchGraphNode[];
  graphEdges: ResearchGraphEdge[];
  warnings: string[];
  modelId: number;
}
