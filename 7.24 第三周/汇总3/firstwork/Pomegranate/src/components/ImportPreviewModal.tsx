...
interface Props {
  ...
  /** 是否为 Markdown/TXT/PDF/Word/Excel 混合导入。 */
  mixed?: boolean;
  onCancel: () => void;
...
export function ImportPreviewModal({
  ...
  const formatStats = useMemo(() => {
    ...
    for (const file of files) {
      ...
      const label = extension === "markdown" ? "MD" : extension.toUpperCase();
  ...
  return (
    <Modal
      open={open}
      title={mixed ? "导入混合文档" : "导入 Markdown"}
      width={520}
      okText="开始导入"
      cancelText="取消"
      onCancel={onCancel}
      ...
      <div className="text-[13px] leading-7">
        ...
        {mixed && (
          <div className="mb-3 flex flex-wrap gap-1.5">
            {formatStats.map(([format, count]) => (
              <Tag key={format} color="blue" style={{ margin: 0 }}>
                {format} {count}
              </Tag>
            ))}
          </div>
        ...
        >
          <div className="flex items-center gap-1.5 font-medium mb-0.5">
            ...
            <span>
              {mixed ? "已按文件类型自动识别并保留目录层级" : "已自动启用 Obsidian 仓库兼容"}
            </span>
          </div>
          <div className="opacity-75">
            {mixed ? (
              <>
                支持 Markdown、TXT、PDF、DOC、DOCX、XLSX、XLS、XLSM、XLSB、ODS · 跳过隐藏目录和 node_modules ·
                PDF/Word/Excel 会在真正导入时进行内容解析与重复检查
              </>
            ) : (
              <>
