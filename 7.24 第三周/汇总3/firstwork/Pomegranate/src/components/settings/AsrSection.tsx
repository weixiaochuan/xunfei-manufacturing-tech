 ...
 *
 * - API Key 只写入后端 secure-credentials；SQLite 仅保存 credentialId 和状态
...
export function AsrSection() {
  ...
  async function handleSave() {
    if (cfg.enabled && !cfg.apiKey.trim() && !cfg.credentialConfigured) {
  ...
  async function handleTest() {
    if (!cfg.apiKey.trim() && !cfg.credentialConfigured) {
