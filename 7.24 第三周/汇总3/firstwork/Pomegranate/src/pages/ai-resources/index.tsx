...
export default function AiResourcesPage() {
  ...
  const activePipelineRef = useRef<ActiveAgentPipeline | null>(null);
    ...
    listen<AgentStreamEvent>("agent:stream", async (event) => {
      ...
      if (payload.event === "completed" || payload.event === "cancelled" || payload.event === "error") {
        ...
        if (
          ...
          && active.rawOutput.trim()
        ) {
          const processed = await runPluginPipelineAfterModel(active.before, active.rawOutput);
          if (processed.output !== active.rawOutput) {
            try {
              ...
              setStreamingText(processed.output);
            } catch (error) {
              message.warning(`插件后处理结果未能写入会话：${String(error)}`);
            }
          }
          if (processed.warnings.length > 0) {
            message.warning(`有 ${processed.warnings.length} 个智能体增强步骤未完全执行`);
          }
        }
  ...
  async function sendMessage() {
    ...
    try {
      const before = await runPluginPipelineBeforeModel({
        scene: "global",
        feature: "external-agent-chat",
        input: content,
        sessionId: selectedSessionId,
        workspaceId: selectedAgent ? `external-agent:${selectedAgent.id}` : undefined,
        metadata: {
          externalAgentId: selectedAgent?.id,
          provider: selectedAgent?.provider,
          protocolType: selectedAgent?.protocolType,
          mockMode: selectedAgent?.mockMode ?? false,
        },
      });
      const result = await externalAgentApi.sendMessage({
        sessionId: selectedSessionId,
        content,
        effectiveContent: pluginPipelineTextInput(before, content),
      ...
      if (before.warnings.length > 0) {
        message.warning(`有 ${before.warnings.length} 个智能体增强步骤未完全执行，已继续调用`);
      }
      ...
      activePipelineRef.current = null;
      setInput(content);
      message.error(normalizeWorkflowException(error));
    }
  }
