...
interface ActiveMobileAiPluginPipeline {
...
export function MobileAiChat() {
  ...
  const activePipelineRef = useRef<ActiveMobileAiPluginPipeline | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      const tokenU = await listen<string>("ai:token", (e) => {
        ...
        if (!active || active.conversationId !== convIdRef.current) return;
      ...
      const doneU = await listen<number | string>("ai:done", async (event) => {
        setStreaming(false);
        setStreamingText("");
        ...
        if (
          ...
          && active?.conversationId === completedConversationId
        ) {
          ...
          const rawOutput = [...persisted]
            ...
            ?.content ?? "";
          if (rawOutput.trim()) {
            const processed = await runPluginPipelineAfterModel(active.before, rawOutput);
        ...
        if (
          completedConversationId === null
          || completedConversationId === convIdRef.current
        ) {
          await loadAll();
        }
      });
  ...
  async function send() {
    ...
    try {
      const before = await runPluginPipelineBeforeModel({
        ...
        workspaceId: `ai-conversation:${convId}`,
        metadata: {
          modelId: model?.id,
