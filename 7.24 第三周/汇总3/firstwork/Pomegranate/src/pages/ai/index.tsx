...
interface ActiveAiPluginPipeline {
...
function DesktopAiChatPage() {
  ...
  const activePipelineRef = useRef<ActiveAiPluginPipeline | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      const tokenUnlisten = await listen<string>("ai:token", (event) => {
        ...
        if (
          !activePipeline
          || activePipeline.conversationId !== activeConvIdRef.current
        ) {
          return;
        }
        setStreamingText((prev) => prev + event.payload);
      });
      const doneUnlisten = await listen<number | string>("ai:done", async (event) => {
        setStreaming(false);
        ...
        if (
          ...
          && activePipeline?.conversationId === completedConversationId
        ) {
          activePipelineRef.current = null;
          const persisted = await aiChatApi.listMessages(completedConversationId);
          const rawOutput = [...persisted]
            .reverse()
            .find((item) => item.role === "assistant")
            ?.content ?? "";
          if (rawOutput.trim()) {
            const processed = await runPluginPipelineAfterModel(
              activePipeline.before,
              rawOutput,
            );
            if (processed.output !== rawOutput) {
              try {
                await aiChatApi.finalizePluginOutput(
                  completedConversationId,
                  rawOutput,
                  processed.output,
                );
              } catch (error) {
                message.warning(`插件后处理结果未能写入会话：${String(error)}`);
              }
            }
            if (processed.warnings.length > 0) {
              message.warning(`有 ${processed.warnings.length} 个对话增强步骤未完全执行`);
            }
          }
        ...
        if (conv && (completedConversationId === null || completedConversationId === conv)) {
  ...
  const handleSend = useCallback(async (textOverride?: string) => {
    ...
    try {
      ...
      const activeModel = models.find((item) => item.id === activeConversation?.model_id);
      const before = await runPluginPipelineBeforeModel({
        scene: "global",
        feature: "ai-chat-message",
        input: text,
        sessionId: String(activeConvId),
        workspaceId: `ai-conversation:${activeConvId}`,
        metadata: {
      ...
      await aiChatApi.sendMessage(
        ...
        pluginPipelineTextInput(before, text),
        before.prompt || undefined,
      );
      if (before.warnings.length > 0) {
        message.warning(`有 ${before.warnings.length} 个对话增强步骤未完全执行，已继续原对话`);
      }
        ...
        {!activeConvId ? (
          ...
          <>
            ...
            >
              ...
              {streaming && (() => {
                const cleanText = stripPlanningUpdateBlock(stripPseudoToolCalls(streamingText));
              ...
              <div
                className="w-[360px] shrink-0 p-3 overflow-auto hidden xl:block"
                style={{ borderLeft: `1px solid ${token.colorBorderSecondary}` }}
              >
                <PlanningWithFilesPanel
                  sessionKind="ai"
                  sessionId={activeConvId ? String(activeConvId) : null}
                  disabled={streaming}
                />
              </div>
            </div>
  ...
  return (
    ...
    >
      ...
      <div className={`max-w-[75%] min-w-0 flex flex-col gap-2 ${isUser ? "items-end" : "items-start"}`}>
        ...
        >
          {isUser ? msg.content : <Markdown>{stripPlanningUpdateBlock(msg.content)}</Markdown>}
        </div>
