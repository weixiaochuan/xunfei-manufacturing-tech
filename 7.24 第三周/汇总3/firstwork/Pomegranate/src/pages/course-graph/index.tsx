            ...
            <Space direction="vertical" className="w-full" size="middle">
              ...
              {selectedNode.type === "Knowledge" && (
                <Space wrap>
                  ...
                  {aiAnalysis?.relations.some((item) => item.status === "accepted") && (
                    <Button
                      size="small"
...
function AiAnalysisBlock({
  ...
  if (!analysis) {
    return (
      <Card size="small" title="AI 知识点分析">
        <Empty description="尚未生成 AI 分析" image={Empty.PRESENTED_IMAGE_SIMPLE}>
          <Button type="primary" icon={<BrainCircuit size={14} />} loading={analyzing} onClick={onAnalyze}>
            生成分析与关系建议
          </Button>
        </Empty>
      </Card>
  ...
  const pendingRelations = analysis.relations.filter((item) => item.status === "pending");
  const reviewedRelations = analysis.relations.filter((item) => item.status !== "pending");
  ...
  return (
    <Card
      ...
      title="AI 知识点分析"
      extra={
        <Button size="small" icon={<BrainCircuit size={14} />} loading={analyzing} onClick={onAnalyze}>
          重新分析
        </Button>
      }
    >
      <Space direction="vertical" className="w-full" size="middle">
        <Alert
          showIcon
          type="info"
          message="AI 结果仅作为候选建议"
          description="关系建议默认处于 pending 状态，只有手动接受后才会作为 AI overlay 展示，不会直接改写只读课程图谱资源。"
        />
        <div>
          <Typography.Text strong>定义</Typography.Text>
          <Typography.Paragraph className="mt-2 rounded-lg bg-slate-50 p-3">
            {analysis.definition || "暂无定义"}
          </Typography.Paragraph>
        </div>
        <div>
          <Typography.Text strong>学习摘要</Typography.Text>
          <Typography.Paragraph className="mt-2 rounded-lg bg-slate-50 p-3">
            {analysis.summary || "暂无摘要"}
          </Typography.Paragraph>
        </div>
        <TagGroup title="别名" values={analysis.aliases} />
        <TagGroup title="前置知识" values={analysis.prerequisites} />
        <TagGroup title="典型应用" values={analysis.applications} />
        <TagGroup title="常见误区" values={analysis.misconceptions} />
        <div>
          <Typography.Text strong>候选关系</Typography.Text>
          {analysis.relations.length === 0 ? (
            <Empty className="mt-2" image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无关系建议" />
          ) : (
            <List
              className="mt-2"
              size="small"
              dataSource={[...pendingRelations, ...reviewedRelations]}
              renderItem={(relation) => (
                <List.Item
                  actions={
                  ...
                  <List.Item.Meta
                    title={
                      <Space wrap size="small">
                        <Typography.Text>{relation.targetNodeName}</Typography.Text>
                        <Tag color="purple">{relation.relationType}</Tag>
                        <Tag
                          color={
                        ...
                        <Tag>置信度 {(relation.confidence * 100).toFixed(0)}%</Tag>
                      </Space>
                    }
