...
import { ArrowLeft, Save, Trash2, Pin, FolderOpen, Tags, Link2, Share, Maximize2, Minimize2, FileText as FileTextIcon, ChevronRight, ChevronDown, CornerUpLeft, Folder as FolderIcon, Eye, EyeOff, Lock, Unlock, MessageSquare, ListTree, Network, ExternalLink, WandSparkles } from "lucide-react";
import { CloseCircleFilled } from "@ant-design/icons";
import { useAppStore } from "@/store";
import { useTabsStore } from "@/store/tabs";
import { noteApi, tagApi, folderApi, linkApi, exportApi, sourceFileApi, vaultApi, sourceWritebackApi, pdfApi } from "@/lib/api";
...
import { ExcelMarkdownSourceEditor, isExcelSourceType } from "@/components/excel/ExcelMarkdownSourceEditor";
...
function DesktopNoteEditorPage() {
  ...
  const isExcelNote = isExcelSourceType(note?.source_file_type);
  /**
   * 思维导图打开时自动暂时隐藏大纲——三栏（编辑器+大纲+导图）会挤压编辑可读宽度。
   * 不动用户在 store 里的偏好；mindMapOpen 切回 false 后大纲自动恢复。
   */
  const effectiveOutlineVisible = outlineVisible && !mindMapOpen && !isExcelNote;
  ...
  function handleRebuildPdfEditable() {
    Modal.confirm({
      title: "重新整理为可编辑正文？",
      content: (
        <div>
          <p>应用将重新读取保留的原始 PDF，识别标题、段落、列表、分页和可提取图片。</p>
          <p style={{ marginBottom: 0 }}>
            当前正文会被覆盖；文档标题、目录、标签和原始 PDF 不会改变。
          </p>
        </div>
      ),
      okText: "重新整理",
      ...
      onOk: async () => {
        try {
          await pdfApi.rebuildEditableNote(noteId);
          clearDraft(noteId);
          await loadData();
          setTabDirty(noteId, false);
          useAppStore.getState().bumpNotesRefresh();
          message.success("PDF 已重新整理为可编辑正文，并提取可识别图片");
        } catch (error) {
          message.error(`PDF 正文整理失败：${String(error)}`);
          throw error;
        }
      },
    });
      ...
      <div className="editor-topbar">
        ...
        <Space align="center">
          ...
          {note?.source_file_type === "pdf" && (
            <Tooltip title="从原始 PDF 重新识别文字结构和可提取图片，并覆盖当前正文">
              <Button
                icon={<WandSparkles size={16} />}
                onClick={handleRebuildPdfEditable}
              >
                整理正文
              </Button>
            </Tooltip>
        ...
        <div className="editor-content-area">
          ...
          <div className="editor-meta">
            <MetaBar
              ...
              editor={isExcelNote ? null : editorInstance}
