...
mod tests {
    ...
    #[test]
    fn plugin_output_finalize_only_updates_matching_latest_assistant_message() {
        let db = temp_db();
        let model = db.create_ai_model(&input("pipeline-model")).unwrap();
        let conversation = db
            .create_ai_conversation("pipeline-test", model.id)
            .unwrap();
        db.add_ai_message(conversation.id, "user", "original user input", None)
            .unwrap();
        db.add_ai_message(conversation.id, "assistant", "raw model output", None)
            .unwrap();

        assert!(db
            .finalize_ai_plugin_output(
                conversation.id,
                "raw model output",
            ...
            .unwrap());
        assert_eq!(
            db.list_ai_messages(conversation.id)
                .unwrap()
                .last()
                .unwrap()
                .content,
...
impl Database {
    ...
    /// 仅在最新 assistant 回复仍等于模型原始输出时写入插件后处理结果。
    ///
    /// 这个比较并更新操作放在同一条参数化 SQL 中，避免较晚完成的流水线覆盖
    /// 用户切换会话后产生的新回复。
    pub fn finalize_ai_plugin_output(
        ...
        let changed = conn.execute(
            "UPDATE ai_messages
             ...
             AND content = ?2",
            rusqlite::params![conversation_id, expected_output, final_output],
        )?;
        Ok(changed == 1)
    }
