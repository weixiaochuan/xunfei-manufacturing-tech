...
use crate::services::{planning::PlanningService, skills};
...
fn planning_update_tool_schema() -> Value {
    json!({
        "type": "function",
        "function": {
            "name": "planning_update",
            "description": "Update the current Planning with Files workspace. Use when goal, acceptance criteria, stage, todos, findings, decisions, blockers, progress, or next step changes. Do not expose this call to the user.",
            "parameters": {
                "type": "object",
                "properties": {
                    "goal": { "type": "string" },
                    ...
                    "stage": { "type": "string" },
                    "todoUpdates": { "type": "array", "items": { "type": "string" } },
...
impl AiService {
    ...
    pub async fn chat_stream(
        ...
        use_rag: bool,
        planning_system_context: Option<&str>,
        plugin_system_context: Option<&str>,
        cancel_rx: watch::Receiver<bool>,
    ) -> Result<(), AppError> {
        ...
        if use_rag {
            let notes = db.search_notes_for_rag(effective_user_message, RAG_TOP_N)?;
            if !notes.is_empty() {
                let total_budget =
                ...
                for (id, title, content) in &notes {
                    ...
                    let snippet = if plain_chars <= single_max {
                        ...
                        extract_window_for_rag(&plain, effective_user_message, single_max)
                    };
        ...
        let mut history = db.list_ai_messages(conversation_id)?;
        if let Some(message) = history.iter_mut().find(|message| message.id == user_msg.id) {
            message.content = effective_user_message.to_string();
        }
        ...
        for &max_hist in &max_history_attempts {
            ...
            match result {
                Ok(response) => {
                    ...
                    let auto_title = derive_conversation_title(visible_user_message);
    ...
    fn build_messages(
        ...
        if let Some(extra) = planning_system_context
            .map(str::trim)
            .filter(|s| !s.is_empty())
        {
            ...
            system_prompt.push_str("\n[/HIDDEN PLANNING CONTEXT]\n");
            system_prompt.push_str(PlanningService::planning_tool_instruction());
        }
        if let Some(extra) = plugin_system_context
            .map(str::trim)
            .filter(|s| !s.is_empty())
        {
                        ...
                        Some(Ok(bytes)) => {
                            ...
                            for line in text.lines() {
                                ...
                                if let Ok(data) = serde_json::from_str::<Value>(line) {
                                    if let Some(content) = data["message"]["content"].as_str() {
                                        full_response.push_str(content);
                                        let visible = PlanningService::sanitize_visible_response(content);
                                        if !visible.is_empty() {
                                            let _ = app.emit("ai:token", visible);
                                        }
                                    }
                                    if data["done"].as_bool() == Some(true) {
                                        return Ok(full_response);
    ...
    pub async fn chat_stream_with_skills(
        ...
        // 2. 保存用户消息
        let user_msg = db.add_ai_message(conversation_id, "user", visible_user_message, None)?;
        db.touch_ai_conversation(conversation_id)?;
        ...
        if !messages.iter().rev().any(|m| {
            m["role"].as_str() == Some("user")
                && m["content"].as_str() == Some(effective_user_message)
        }) {
            messages.push(json!({ "role": "user", "content": effective_user_message }));
        }
        ...
        let mut tool_schemas = skills::tool_schemas_with_mcp(&app).await;
        if planning_system_context.is_some() {
            tool_schemas.push(planning_update_tool_schema());
        }
        ...
        for round in 0..=MAX_TOOL_ROUNDS {
            ...
            if tool_calls.is_empty() {
                ...
                let stripped =
                    PlanningService::sanitize_visible_response(&strip_pseudo_tool_calls(&content));
            ...
            for tc in &tool_calls {
                ...
                let (result_text, status) = if tc.name == "planning_update" {
                    match PlanningService::apply_tool_update_and_emit(
                        &app,
                        db,
                        data_dir,
                        crate::models::PlanningSessionKind::Ai,
                        ...
                        Ok(workspace) => (
                            json!({
                                "ok": true,
                                "revision": chrono::Local::now().timestamp_millis(),
                        ...
                        Err(e) => (format!("planning_update failed: {}", e), "error"),
                    }
                } else {
                ...
                // 回注给模型
                if planning_system_context.is_some() {
                    PlanningService::record_activity_and_emit(
                        &app,
                        db,
                        data_dir,
                        crate::models::PlanningSessionKind::Ai,
                        &conversation_id.to_string(),
                        "tool",
                        &format!("AI tool `{}` completed with status {}", tc.name, status),
                    )
                    .ok();
                }
                messages.push(json!({
...
/// 两处都要调用，避免重复。
#[cfg(test)]
mod plugin_pipeline_message_tests {
    use super::*;
    ...
    #[test]
    fn plugin_context_is_a_hidden_system_section_not_user_content() {
        let history = vec![AiMessage {
            id: 1,
            conversation_id: 1,
            role: "user".into(),
            content: "effective model input".into(),
            references: None,
            skill_calls: None,
            created_at: String::new(),
        }];

        let messages = AiService::build_messages(
            &model(),
            &history,
            "",
            "",
            None,
            Some("declarative plugin instruction"),
            20,
        );

        assert_eq!(messages[1]["content"], "effective model input");
        let system = messages[0]["content"].as_str().unwrap();
        assert!(system.contains("[HIDDEN PLUGIN CONTEXT]"));
        assert!(system.contains("declarative plugin instruction"));
        assert!(!messages[1]["content"]
            .as_str()
            .unwrap()
            .contains("declarative plugin instruction"));
    }

    #[test]
    fn disabled_plugin_context_leaves_model_messages_unchanged() {
        let history = vec![AiMessage {
            id: 1,
            conversation_id: 1,
            role: "user".into(),
            content: "plain input".into(),
            references: None,
            skill_calls: None,
            created_at: String::new(),
        }];

        let messages = AiService::build_messages(&model(), &history, "", "", None, None, 20);

        assert_eq!(messages[1]["content"], "plain input");
        assert!(!messages[0]["content"]
            .as_str()
            .unwrap()
            .contains("[HIDDEN PLUGIN CONTEXT]"));
    }
}
...
fn handle_openai_stream_line(
    ...
    if let Some(c) = delta["content"].as_str() {
        content.push_str(c);
        let visible = PlanningService::sanitize_visible_response(c);
        if !visible.is_empty() {
            let _ = app.emit("ai:token", visible);
        }
    }
