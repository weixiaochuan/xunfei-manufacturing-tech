...
use crate::models::course_graph_ai::{
    CourseGraphAiModelOutput, CourseGraphAiRelation,
};
use crate::models::{
    CourseGraphAiAnalysis, PluginAiChatInput, PluginAiMessage,
};
...
{{
  ...
  "relations": [
    {{"targetNodeId":"候选 id","relationType":"关系类型","reason":"判断依据","confidence":0.85}}
        ...
        let request_id = format!("course-graph-ai-{}-{}", node_id, chrono::Utc::now().timestamp_millis());
    ...
    pub fn review_relation(
        ...
        if !matches!(status, "accepted" | "rejected" | "pending") {
            return Err(AppError::InvalidInput("关系状态只能是 pending、accepted 或 rejected".into()));
        }
        db.review_course_graph_ai_relation(relation_id, status)
    }

    pub fn accepted_graph(app: &AppHandle, db: &Database, node_id: &str) -> Result<Value, AppError> {
        ...
        for relation in relations {
            if let Ok(target) = CourseGraphService::node_detail(app, relation.target_node_id.clone()) {
                nodes.push(target);
                edges.push(json!({
