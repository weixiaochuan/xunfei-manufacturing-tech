...
use crate::models::{NoteInput, TaskQuery};
...
const NOTE_WRITE_MAX_CHARS: usize = 80_000;
    ...
    vec![
        ...
        json!({
            ...
            "function": {
                ...
                "description": "Create a Markdown note inside firstwork. Use only when the user explicitly asks to create, save, or generate an MD document. This writes to the controlled notes database, not to an arbitrary filesystem path.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "title": { "type": "string", "description": "Non-empty note title" },
                        "content": { "type": "string", "description": "Markdown body" },
                        "folder_id": { "type": ["integer", "null"], "description": "Optional firstwork folder id; null means root" }
                    },
        ...
        json!({
            ...
            "function": {
                ...
                "description": "Replace the Markdown body of an existing firstwork note. Use only when the user explicitly asks to edit a known note_id. Hidden or encrypted notes are refused.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "note_id": { "type": "integer", "description": "Target note id" },
                        "title": { "type": ["string", "null"], "description": "Optional new title; null keeps the current title" },
                        "content": { "type": "string", "description": "Replacement Markdown body" },
                        "folder_id": { "type": ["integer", "null"], "description": "Optional new folder id; null keeps current folder" }
                    },
...
pub fn dispatch(db: &Database, name: &str, args_json: &str) -> Result<String, AppError> {
    let result = match name {
        ...
        "create_markdown_note" => run_create_markdown_note(db, args_json)?,
        "update_note_markdown" => run_update_note_markdown(db, args_json)?,
        other => {
...
pub async fn dispatch_with_mcp(
    ...
    let result = dispatch(db, name, args_json)?;
    #[cfg(desktop)]
    if matches!(name, "create_markdown_note" | "update_note_markdown") {
        if let Ok(payload) = serde_json::from_str::<Value>(&result) {
            if name == "create_markdown_note" {
                let _ = app.emit("quick_capture:note_created", payload);
            } else if let Some(id) = payload.get("id").and_then(|v| v.as_i64()) {
                let _ = app.emit(
                    "note:updated",
                    json!({
                        "id": id,
                        "sourceLabel": "ai-skill",
                    }),
                );
            }
        }
...
fn run_create_markdown_note(db: &Database, args: &str) -> Result<String, AppError> {
    let a: CreateMarkdownNoteArgs = serde_json::from_str(args)
        .map_err(|e| AppError::Custom(format!("create_markdown_note args invalid: {}", e)))?;
    let title = a.title.trim();
    if title.is_empty() {
        return Err(AppError::InvalidInput("note title cannot be empty".into()));
    }
    validate_markdown_write_content(&a.content)?;
    ...
    Ok(serde_json::to_string(&json!({
        ...
        "folder_id": note.folder_id,
        "updated_at": note.updated_at,
        "message": "Markdown note created"
    }))
    .unwrap_or_else(|_| r#"{"ok":true}"#.to_string()))
}

#[derive(Debug, Deserialize)]
struct UpdateNoteMarkdownArgs {
    note_id: i64,
    #[serde(default)]
    title: Option<String>,
    content: String,
...
fn run_update_note_markdown(db: &Database, args: &str) -> Result<String, AppError> {
    let a: UpdateNoteMarkdownArgs = serde_json::from_str(args)
        .map_err(|e| AppError::Custom(format!("update_note_markdown args invalid: {}", e)))?;
    validate_markdown_write_content(&a.content)?;

    let current = db
        .get_note(a.note_id)?
        .ok_or_else(|| AppError::NotFound(format!("note {} not found", a.note_id)))?;
    if current.is_hidden {
        return Err(AppError::Custom(
            "AI cannot update hidden notes".to_string(),
        ));
    }
    if current.is_encrypted {
        return Err(AppError::Custom(
            "AI cannot update encrypted notes".to_string(),
        ));
    }
    ...
    let note = db.update_note(
        ...
        &NoteInput {
            title,
            content: a.content,
            folder_id: a.folder_id.or(current.folder_id),
        },
    )?;
    ...
    Ok(serde_json::to_string(&json!({
        ...
        "folder_id": note.folder_id,
        "updated_at": note.updated_at,
        "message": "Markdown note updated"
    }))
    .unwrap_or_else(|_| r#"{"ok":true}"#.to_string()))
}

fn validate_markdown_write_content(content: &str) -> Result<(), AppError> {
    let chars = content.chars().count();
    if chars == 0 {
        return Err(AppError::InvalidInput(
            "Markdown content cannot be empty".into(),
        ));
    }
    if chars > NOTE_WRITE_MAX_CHARS {
        return Err(AppError::InvalidInput(format!(
            "Markdown content is too long ({} chars), max is {} chars",
            chars, NOTE_WRITE_MAX_CHARS
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn markdown_write_rejects_empty_content() {
        assert!(validate_markdown_write_content("").is_err());
    }

    #[test]
    fn markdown_write_rejects_oversized_content() {
        let content = "a".repeat(NOTE_WRITE_MAX_CHARS + 1);
        assert!(validate_markdown_write_content(&content).is_err());
    }
}
