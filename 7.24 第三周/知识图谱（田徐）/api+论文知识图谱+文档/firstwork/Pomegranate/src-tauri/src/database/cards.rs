...
impl Database {
    ...
    pub fn create_card(&self, input: &CreateCardInput) -> Result<Card, AppError> {
        ...
        drop(conn);
        self.get_card(id)?
            .ok_or_else(|| AppError::Custom(format!("卡片创建后查询失败: id={}", id)))
    }
    ...
    pub fn get_card(&self, id: i64) -> Result<Option<Card>, AppError> {
        ...
        let card = stmt.query_row(params![id], |row| to_card(row)).ok();
        Ok(card)
    }
    ...
    /// 更新卡片正反面
    pub fn update_card_content(&self, id: i64, front: &str, back: &str) -> Result<(), AppError> {
        let conn = self.conn_lock()?;
        conn.execute(
