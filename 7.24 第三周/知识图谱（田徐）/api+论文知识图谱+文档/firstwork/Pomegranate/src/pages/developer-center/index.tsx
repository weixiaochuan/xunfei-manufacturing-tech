...
function RoleSwitcher({ session, onChanged }: { session: MarketplaceMockSession | null; onChanged: () => void }) {
  ...
  return (
    <Alert
      ...
      showIcon
      className="mb-4"
      message="本地演示角色，不代表真实登录"
      description={
        <Space wrap>
          <Text>当前：{session?.displayName ?? "未知"} / {session?.role ?? "-"}</Text>
          <Button size="small" onClick={async () => { await marketplaceApi.switchMockRole("customer"); onChanged(); }}>切到买家</Button>
          <Button size="small" type="primary" onClick={async () => { await marketplaceApi.switchMockRole("developer"); onChanged(); }}>切到创作者</Button>
          <Button size="small" onClick={async () => { await marketplaceApi.switchMockRole("admin"); onChanged(); }}>切到管理员</Button>
        </Space>
      }
    />
  );
...
export default function DeveloperCenterPage() {
  ...
  async function uploadZip() {
    ...
    try {
      ...
      setReport(result);
      if (result.ok) {
        message.success("上传检查通过，已保存到受控市场资产目录");
      } else {
        message.error("上传检查失败，未创建可提交版本");
      }
      await load();
      setUploadOpen(false);
    } finally {
  ...
  return (
    <div className="commerce-page">
      ...
      <RoleSwitcher session={session} onChanged={() => { void load("role-switch"); }} />
      <CommerceHero
        eyebrow="CREATOR STUDIO / LOCAL DEMO"
        title="开发者中心"
        description="创建工作流/API/MCP 调用项，Pomegranate 只保存展示信息、Schema 与安全连接配置；星辰工作流本体、授权和实际运行仍在讯飞星辰平台完成。"
        icon={<Boxes size={27} />}
        badge={<Tag color="cyan">{session?.displayName ?? "本地创作者"}</Tag>}
        actions={(
            ...
            children: visitedTabs.has("products") ? (
              <div className="commerce-master-detail">
                  ...
                  <Card
                    ...
                    extra={<Button disabled={!selected} onClick={() => { setUploadVersion(latest?.version ? bumpPatch(latest.version) : "1.0.0"); setUploadOpen(true); }} loading={loading}>上传 .zip 版本</Button>}
                  >
                    {selected ? (
                      <>
      ...
      <Modal
        ...
        okText="选择 .zip 并上传"
      >
        <Paragraph>manifest.id 必须等于当前商品 ID，manifest.version 必须等于下方版本号。</Paragraph>
...
function bumpPatch(version: string) {
  ...
  if (parts.length !== 3 || parts.some((v) => Number.isNaN(v))) return "1.0.0";
  parts[2] += 1;
  return parts.join(".");
}
