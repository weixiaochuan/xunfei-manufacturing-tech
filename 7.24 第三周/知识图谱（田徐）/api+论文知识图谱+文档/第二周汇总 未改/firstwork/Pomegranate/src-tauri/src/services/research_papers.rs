...
const SEMANTIC_SCHOLAR_SEARCH_URL: &str =
...
const EUROPE_PMC_SEARCH_URL: &str =
    "https://www.ebi.ac.uk/europepmc/webservices/rest/search";
const PUBMED_SEARCH_URL: &str =
    "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi";
const PUBMED_SUMMARY_URL: &str =
    ...
    match search_semantic_scholar_relevance(query, from_year, to_year).await {
        ...
        Err(primary_error) => search_semantic_scholar_bulk(query, from_year, to_year)
            ...
            .map_err(|fallback_error| {
                format!(
                    "普通检索不可用（{primary_error}）；备用检索也失败（{fallback_error}）"
                )
            }),
    }
}
...
async fn search_europe_pmc(
    ...
    let query_with_date = format!(
        "({query}) AND FIRST_PDATE:[{from_year}-01-01 TO {to_year}-12-31]"
    );
...
fn semantic_bulk_relevance(
    ...
    let abstract_matches = terms
        ...
        .count() as f64;
    let lexical = ((title_matches * 3.0 + abstract_matches) / (terms.len() as f64 * 4.0))
        .clamp(0.0, 1.0);
...
fn arxiv_candidate(
    ...
    let doi = item
        ...
        .and_then(normalized_nonempty)
        .map(|doi| normalize_doi(&doi));
    let url = doi.as_deref().map(doi_url).unwrap_or_else(|| arxiv_url.clone());
...
fn pubmed_candidate(
    ...
    Some(PaperCandidate {
        ...
        paper: ResearchPaper {
            id: doi
                .clone()
                .unwrap_or_else(|| format!("pubmed:{uid}")),
            title,
            authors,
            publication_year,
...
fn dblp_candidate(
    ...
    Some(PaperCandidate {
        ...
        paper: ResearchPaper {
            id: doi
                .clone()
                .unwrap_or_else(|| format!("dblp:{}", info.key)),
            title,
            authors,
            publication_year,
    ...
    let mut scored = sentences
        ...
        .map(|(index, sentence)| {
            ...
            let signal_score = signal_terms
                ...
                .count() as i32
                * 2;
            let position_score = if index == 0 { 2 } else if index == 1 { 1 } else { 0 };
...
fn semantic_work_type(publication_types: &[String]) -> String {
    ...
    if normalized
        .iter()
        .any(|value| value.contains("conference"))
    {
        "proceedings-article".to_string()
    } else if normalized
        .iter()
        .any(|value| value.contains("preprint"))
    {
        "posted-content".to_string()
    } else if normalized
        .iter()
        .any(|value| value.contains("book"))
    {
...
mod tests {
    ...
    fn merges_same_paper_and_preserves_all_sources() {
        ...
        let papers = merge_and_rank_papers(
    ...
    fn parses_new_platform_payloads_and_builds_https_source_links() {
        ...
        let europe_candidate =
            europe_pmc_candidate(europe.result_list.results.into_iter().next().unwrap(), 2024, 2026, 0.9)
        ...
        let dblp_candidate =
            dblp_candidate(dblp.result.hits.items.into_iter().next().unwrap().info, 2024, 2026, 0.9)
    ...
    fn builds_arxiv_candidate_from_openalex_fallback_payload() {
        ...
        let candidate = openalex_arxiv_candidate(
