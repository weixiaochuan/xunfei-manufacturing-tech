...
pub async fn check_cli() -> Result<String, AppError> {
    ...
    let output = cmd
        .output()
        .await
        .map_err(|e| AppError::Custom(format!("找不到 {}: {}", cli, e)))?;
...
pub async fn start_session(
    ...
    if !is_path_safe(&input.project_path, &data_dir) {
        return Err(AppError::Custom(
            "不允许在此目录中运行 Claude Code。请选择有效的项目目录。".into(),
        ));
    }
            ...
            if let Some(s) = existing {
                if s.project_path == input.project_path && s.status == "running" {
                    return Err(AppError::Custom(format!(
                        "项目 {} 中已有运行中的 Agent 会话 ({}), 请先停止后再启动",
                        input.project_path, s.id
                    )));
                }
            }
        }
    ...
    let mut child = cmd
        .spawn()
        .map_err(|e| AppError::Custom(format!("无法启动 {}: {}", cli, e)))?;
