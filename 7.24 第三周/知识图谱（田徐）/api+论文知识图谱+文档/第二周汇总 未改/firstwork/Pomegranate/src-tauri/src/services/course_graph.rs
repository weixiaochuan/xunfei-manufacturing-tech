...
const RESOURCE_DB_RELATIVE: &str = "resources/process_graph.db";
const RESOURCE_DB_NAME: &str = "process_graph.db";
...
impl CourseGraphService {
    ...
    /// 为 AI 分析提供受控候选集。模型只能从这些真实节点中选择目标，避免生成不存在的知识点 ID。
    pub fn analysis_context(
        ...
        let node_id = checked_node_id(&node_id, "nodeId")?;
        let path = resolve_resource_db_path(app)?;
        analysis_context_from_path(&path, &node_id, limit.unwrap_or(16).clamp(4, 30))
    }
}
...
fn resolve_resource_db_path(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(resource_dir) = app.path().resource_dir() {
        ...
        candidates.push(resource_dir.join(RESOURCE_DB_NAME));
    }
...
fn resource_db_candidates() -> Vec<PathBuf> {
    ...
    vec![
        ...
        manifest_dir.join(RESOURCE_DB_NAME),
    ]
}
...
fn node_detail_from_path(path: &Path, node_id: &str) -> Result<Value, String> {
    ...
    Ok(json!({
        ...
        "labels": node.get("labels").cloned().unwrap_or_else(|| json!(["Entity"])),
...
fn analysis_context_from_path(path: &Path, node_id: &str, limit: u32) -> Result<Value, String> {
    let conn = open_readonly(path)?;
    let node = get_node(&conn, node_id)?.ok_or_else(|| "课程图谱节点不存在".to_string())?;
    if node.get("nodeType").and_then(Value::as_str) != Some("Knowledge") {
        return Err("目前仅支持分析知识点节点".to_string());
    }
    let section_id = node.get("sectionId").and_then(Value::as_str).unwrap_or_default();
    let chapter_id = node.get("chapterId").and_then(Value::as_str).unwrap_or_default();
    let mut stmt = conn
        .prepare(
            r#"
            SELECT n.id, n.node_type, n.name, n.content, n.chapter_id, n.section_id, n.metadata
            FROM nodes n
            WHERE n.node_type = 'Knowledge'
              AND n.id <> ?1
              AND ((?2 <> '' AND n.section_id = ?2) OR (?2 = '' AND n.chapter_id = ?3))
            ORDER BY CASE WHEN EXISTS (
                SELECT 1 FROM edges e
                WHERE e.relation_type = 'RELATED_TO'
        ...
        .map_err(query_error)?;
    let candidates = stmt
        .query_map(params![node_id, section_id, chapter_id, limit as i64], node_from_row)
        .map_err(query_error)?
        .collect::<Result<Vec<_>, _>>()
        .map_err(query_error)?;
    let version = metadata_string(&conn, "version")?;
    Ok(json!({
        "node": node,
        "candidates": candidates,
        "sourceKind": "bundled-course-graph",
