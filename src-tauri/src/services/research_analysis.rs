...
impl ResearchAnalysisService {
    pub async fn analyze(
        ...
        let model = db.get_default_ai_model().map_err(|_| {
            AppError::InvalidInput(
                "尚未配置默认 AI 模型。请到“设置 → AI能力中心 → AI 模型配置”添加模型、测试连接并设为默认。".into(),
            )
        })?;
...
fn validate_input(input: &ResearchAnalysisInput) -> Result<(), AppError> {
    if !(1..=MAX_PAPERS).contains(&input.file_paths.len()) {
        return Err(AppError::InvalidInput(format!(
            "请上传 1 到 {MAX_PAPERS} 篇 PDF 论文"
        )));
    }
...
fn normalize_result(result: &mut ResearchAnalysisResult, expected_papers: usize) {
    ...
    result
        .papers
        .retain(|paper| allowed_paper_ids.contains(&paper.paper_id));
    ...
    for recommendation in &mut result.recommendations {
        ...
        recommendation
            ...
            .retain(|id| allowed_paper_ids.contains(id));
    }
    enrich_keyword_graph(result);
    let node_ids: HashSet<String> = result
        .graph_nodes
        .iter()
...
fn build_keyword_overlaps(
    ...
    for paper in papers {
        for keyword in &paper.keywords {
            ...
            let entry = occurrences
                .entry(key)
                .or_insert_with(|| (keyword.clone(), Vec::new()));
    ...
    let ai_analysis: HashMap<String, String> = ai_overlaps
        ...
        .map(|item| {
            (
                item.keyword.trim().to_lowercase(),
                item.analysis.trim().to_string(),
            )
        })
        .collect();
...
fn enrich_keyword_graph(result: &mut ResearchAnalysisResult) {
    let mut node_ids: HashSet<String> = result
        .graph_nodes
        .iter()
        .map(|node| node.id.clone())
        .collect();
    let mut edge_ids: HashSet<String> = result
        .graph_edges
        .iter()
...
mod tests {
    ...
    fn shared_keywords_are_normalized_and_added_to_graph() {
        ...
        assert!(result
            .graph_nodes
            .iter()
            .any(|node| node.node_type == "Keyword"));
        assert_eq!(
            result
                .graph_edges
                .iter()
                .filter(|edge| edge.relation_type == "HAS_KEYWORD")
                .count(),
            2
        );
    }
    ...
    #[test]
    fn accepts_single_paper_input_shape() {
        let input = ResearchAnalysisInput {
            file_paths: vec!["paper.pdf".into()],
            project_context: "数控刀架".into(),
        };
        let error = validate_input(&input).expect_err("不存在的文件应在篇数校验之后报错");
        assert!(error.to_string().contains("论文文件不存在"));
    }
}
