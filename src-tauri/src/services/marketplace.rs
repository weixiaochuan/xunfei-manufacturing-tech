...
use crate::models::{
    ...
    McpServerInput, NormalizedPluginManifest, PermissionDiff, PluginClassification,
...
struct VersionRow {
    ...
    classification: Option<PluginClassification>,
...
impl MarketplaceService {
    ...
    pub fn install_product(
        ...
        if version.package_format == "v3-firstwork-plugin" {
            if let Some(existing) = db.get_plugin_installation(&row.plugin_id)? {
                if existing.installed_version == version.version {
                    return Ok(MarketplaceActionResult {
                        ok: true,
                        product_id: row.id,
                        plugin_id: Some(row.plugin_id),
                        message: "该 Manifest v3 版本已经安装".into(),
                        requires_permission_confirmation: false,
                        permission_diff: None,
            ...
            let install = install_v3_marketplace_version(
                db,
                data_dir,
                &row,
                &version,
            ...
            db.write_audit_log(
                &row.plugin_id,
                "marketplace_v3_install",
                Some(&format!("version={}; package hash locked", version.version)),
            )
            .ok();
            return Ok(MarketplaceActionResult {
                ok: true,
                product_id: row.id,
                plugin_id: Some(row.plugin_id),
                message: "Manifest v3 插件已通过统一安全安装核心安装".into(),
                requires_permission_confirmation: false,
                permission_diff: None,
                entitlement: Self::entitlement(db, &input.product_id)?,
    ...
    pub fn uninstall_product(
        ...
        }
        {
            let conn = db.conn_lock()?;
            conn.execute(
                "DELETE FROM plugin_activation_settings WHERE plugin_id = ?1",
    ...
    pub fn update_product(
        ...
        let install = if next.package_format == "v3-firstwork-plugin" {
            install_v3_marketplace_version(
                db,
                data_dir,
                &row,
                &next,
                &next_manifest.permissions,
                app_version,
            )?
        } else {
            let source = package_path(&next)?;
            Self::copy_and_register(db, data_dir, &row, &next, &source, true)?
        };
    ...
    fn active_version(db: &Database, product_id: &str) -> Result<VersionRow, AppError> {
        ...
        conn.query_row(
            "SELECT id, product_id, version, manifest_json, runtime_kind, source,
                    ...
                    package_format, manifest_schema_version, plugin_id, classification,
                    approved_content_hash, package_locked
    ...
    fn summary_for_product(
        ...
        let supported_scenes = if version.package_format == "v3-firstwork-plugin" {
            serde_json::from_str::<PluginManifestV3>(&version.manifest_json)
                .map(|manifest| {
                    manifest
                        .supported_scenes
                        .iter()
...
fn version_row_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<VersionRow> {
    Ok(VersionRow {
        ...
        package_format: row.get(12)?,
        manifest_schema_version: row.get::<_, i64>(13)? as u32,
        plugin_id: row.get(14)?,
        classification: row
            .get::<_, Option<String>>(15)?
            .and_then(|value| serde_json::from_value(json!(value)).ok()),
        approved_content_hash: row.get(16)?,
        package_locked: row.get::<_, i32>(17)? != 0,
    })
}
...
fn manifest_from_row(row: &VersionRow) -> Result<NormalizedPluginManifest, AppError> {
    if row.package_format == "v3-firstwork-plugin" || row.manifest_schema_version == 3 {
        let manifest: PluginManifestV3 = serde_json::from_str(&row.manifest_json)?;
        let product_type = product_type_for_v3_manifest(&manifest);
        let main = manifest
            .contributes
            .features
            .iter()
            .find_map(|feature| feature.ui_schema.clone());
        ...
        let legacy_manifest = crate::models::PluginManifest {
            ...
            main: main.clone().unwrap_or_else(|| "manifest.json".into()),
            styles: None,
        ...
        return Ok(NormalizedPluginManifest {
            format: crate::models::PluginManifestFormat::V3,
...
fn product_type_for_v3_manifest(manifest: &PluginManifestV3) -> ProductType {
...
fn install_v3_marketplace_version(
    db: &Database,
    data_dir: &Path,
    product: &ProductRow,
    version: &VersionRow,
    ...
    if !version.package_locked {
        return Err(AppError::InvalidInput(
            "Manifest v3 版本尚未完成审核包锁定，禁止安装".into(),
        ));
    }
    let approved_hash = version
        .approved_content_hash
        .as_deref()
        .ok_or_else(|| AppError::InvalidInput("Manifest v3 版本缺少审核锁定哈希".into()))?;
    if approved_hash != version.content_hash {
        return Err(AppError::InvalidInput(
            "商品版本哈希与审核锁定哈希不一致，必须重新审核".into(),
        ));
    }
    let archive = version
        .package_path
        .as_ref()
        .map(PathBuf::from)
        .ok_or_else(|| AppError::InvalidInput("Manifest v3 版本缺少安装包路径".into()))?;
    if !archive.is_file() {
        return Err(AppError::NotFound(format!(
            "Manifest v3 安装包不存在：{}",
            archive.display()
    ...
    if actual_hash != approved_hash {
        return Err(AppError::InvalidInput(
            "安装包在批准后发生变化，SHA-256 校验失败".into(),
        ));
    }
    let manifest: PluginManifestV3 = serde_json::from_str(&version.manifest_json)?;
    if manifest.id != product.plugin_id
        || version.plugin_id.as_deref() != Some(manifest.id.as_str())
        || manifest.version != version.version
    {
        return Err(AppError::InvalidInput(
            "市场商品、插件 ID 与批准版本不一致".into(),
        ));
    }
    let result = PluginPlatformService::install_archive(
        db,
        data_dir,
        PluginInstallArchiveInput {
            ...
            // 本地市场在 UI 中明确标记 unsigned；真实签名验证尚未接入。
            confirm_unsigned: true,
        },
        app_version,
    )?;
    let previous_path = db
        .get_plugin_installation(&product.plugin_id)?
        .map(|installation| installation.install_path);
    {
        let conn = db.conn_lock()?;
        conn.execute(
            "INSERT INTO plugin_installations
                (plugin_id, product_id, product_version_id, installed_version, source, enabled,
                 install_path, content_hash, status, previous_install_path, installed_at, updated_at)
             VALUES (?1, ?2, ?3, ?4, 'marketplace', 0, ?5, ?6, 'installed', ?7,
                     datetime('now','localtime'), datetime('now','localtime'))
    ...
    db.get_plugin_installation(&product.plugin_id)?
        .ok_or_else(|| AppError::Custom("Manifest v3 安装记录关联失败".into()))
}
...
fn ensure_version_installable(row: &VersionRow) -> Result<(), AppError> {
    if !matches!(
        row.status.as_str(),
        "active" | "approved" | "published" | "update"
    ) {
        return Err(AppError::InvalidInput(format!(
            "商品版本尚未批准发布，禁止安装: {}",
            row.status
    ...
    fn marketplace_document_summary_plugin_lifecycle_is_usable() {
        ...
        let (_, title, prompt) = PluginService::prepare_document_summary_agent_start(
            ...
            PluginDocumentSummaryAgentStartInput {
                ...
                plugin_system_context: None,
                plugin_contribution_ids: Vec::new(),
            },
        )
        .unwrap();
