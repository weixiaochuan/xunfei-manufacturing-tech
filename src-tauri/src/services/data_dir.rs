...
pub const ACTIVE_ACCOUNT_FILE: &str = "active-account.json";

const ACCOUNT_DIR_ROOT: &str = "accounts";
...
impl DataDirResolver {
    ...
    pub fn resolve(default_app_data_dir: &Path) -> Result<ResolvedDataDir, AppError> {
        ...
        let (base_dir, base_source, pending) = Self::resolve_base(default_app_data_dir)?;

        if let Some(scope) = read_active_account_scope(default_app_data_dir)? {
            let path = account_data_dir(&base_dir, &scope.platform_user_id)?;
            std::fs::create_dir_all(&path)?;
            return Ok(ResolvedDataDir {
                default_dir: default_str.clone(),
                current_dir: path.to_string_lossy().to_string(),
                source: DataDirSource::Account,
                pending_dir: pending,
            });
        }
        ...
        Ok(ResolvedDataDir {
            default_dir: default_str.clone(),
            current_dir: base_dir.to_string_lossy().to_string(),
            source: base_source,
            pending_dir: pending,
        })
    }
    ...
    pub fn resolve_base(
    ...
    ) -> Result<(PathBuf, DataDirSource, Option<String>), AppError> {
        if let Ok(p) = std::env::var("KB_DATA_DIR") {
            ...
            if !trimmed.is_empty() {
                ...
                return Ok((path, DataDirSource::Env, pending));
            }
        }
        ...
        if let Some(target) = read_pointer(default_app_data_dir)? {
            ...
            std::fs::create_dir_all(&path)?;
            return Ok((path, DataDirSource::Pointer, Some(target)));
        }
    ...
    pub fn account_data_dir(
        default_app_data_dir: &Path,
        platform_user_id: &str,
    ) -> Result<PathBuf, AppError> {
        let (base_dir, _, _) = Self::resolve_base(default_app_data_dir)?;
        account_data_dir(&base_dir, platform_user_id)
    }

    pub fn set_active_account_scope(
        default_app_data_dir: &Path,
        platform_user_id: &str,
        account_number: &str,
        username: &str,
    ) -> Result<ActiveAccountScope, AppError> {
        let platform_user_id = require_scope_value("platform_user_id", platform_user_id)?;
        let account_number = require_scope_value("account_number", account_number)?;
        let username = require_scope_value("username", username)?;
        let scope = ActiveAccountScope {
            dir_name: account_dir_name(&platform_user_id)?,
            platform_user_id,
            account_number,
            username,
        };
        let path = default_app_data_dir.join(ACTIVE_ACCOUNT_FILE);
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)?;
        }
        let tmp = default_app_data_dir.join(format!("{}.tmp", ACTIVE_ACCOUNT_FILE));
        let bytes =
            serde_json::to_vec_pretty(&scope).map_err(|e| AppError::Custom(e.to_string()))?;
        std::fs::write(&tmp, bytes)?;
        if path.exists() {
            let _ = std::fs::remove_file(&path);
        }
        std::fs::rename(&tmp, &path)?;
        let account_dir = Self::account_data_dir(default_app_data_dir, &scope.platform_user_id)?;
        std::fs::create_dir_all(account_dir)?;
        Ok(scope)
    }

    pub fn clear_active_account_scope(default_app_data_dir: &Path) -> Result<(), AppError> {
        let marker = default_app_data_dir.join(ACTIVE_ACCOUNT_FILE);
        if marker.exists() {
    ...
    pub fn read_active_account_scope(
        default_app_data_dir: &Path,
    ) -> Result<Option<ActiveAccountScope>, AppError> {
        read_active_account_scope(default_app_data_dir)
    }
...
fn read_active_account_scope(
    ...
    let mut scope: ActiveAccountScope =
        serde_json::from_slice(&bytes).map_err(|e| AppError::Custom(e.to_string()))?;
    let platform_user_id = require_scope_value("platform_user_id", &scope.platform_user_id)?;
    scope.dir_name = account_dir_name(&platform_user_id)?;
    scope.platform_user_id = platform_user_id;
    Ok(Some(scope))
}

fn account_data_dir(base_dir: &Path, platform_user_id: &str) -> Result<PathBuf, AppError> {
    Ok(base_dir
        .join(ACCOUNT_DIR_ROOT)
        .join(account_dir_name(platform_user_id)?))
}

fn account_dir_name(platform_user_id: &str) -> Result<String, AppError> {
    let platform_user_id = require_scope_value("platform_user_id", platform_user_id)?;
    let digest = Sha256::digest(platform_user_id.as_bytes());
    Ok(format!("account-{}", hex_lower(&digest[..16])))
}

fn require_scope_value(name: &str, value: &str) -> Result<String, AppError> {
...
mod tests {
    ...
    #[test]
    fn active_account_scope_uses_separate_account_directory() {
        let app_data = temp_app_data();
        std::env::remove_var("KB_DATA_DIR");

        DataDirResolver::set_active_account_scope(&app_data, "user-a", "POME-001", "alice")
            .unwrap();
        let first = DataDirResolver::resolve(&app_data).unwrap();
        assert_eq!(first.source, DataDirSource::Account);
        assert!(first.current_dir.contains("accounts"));

        DataDirResolver::set_active_account_scope(&app_data, "user-b", "POME-002", "bob").unwrap();
        let second = DataDirResolver::resolve(&app_data).unwrap();
        assert_eq!(second.source, DataDirSource::Account);
        assert_ne!(first.current_dir, second.current_dir);

        DataDirResolver::clear_active_account_scope(&app_data).unwrap();
        let cleared = DataDirResolver::resolve(&app_data).unwrap();
        assert_eq!(cleared.source, DataDirSource::Default);
    }

    #[test]
    fn active_account_scope_honors_custom_data_root() {
        let app_data = temp_app_data();
        let target = temp_app_data().join("custom-root");
        std::env::remove_var("KB_DATA_DIR");
        ...
        let r = DataDirResolver::resolve(&app_data).unwrap();
        assert_eq!(r.source, DataDirSource::Account);
        assert!(PathBuf::from(&r.current_dir).starts_with(target.join("accounts")));
        assert_eq!(r.pending_dir.as_deref(), Some(target.to_str().unwrap()));
    }
