...
struct AiRequestProgressSnapshot {
    response_headers_received: bool,
    first_response_received: bool,
    partial_response_received: bool,
    stream_completed: bool,
    first_response_after_ms: Option<u128>,
    partial_response_after_ms: Option<u128>,
    finish_reason: Option<String>,
    prompt_tokens: Option<u64>,
    completion_tokens: Option<u64>,
    total_tokens: Option<u64>,
    response_characters: Option<usize>,
}

#[derive(Debug, Clone, Default)]
struct AiRequestProgress;

impl AiRequestProgress {
    fn snapshot(&self) -> AiRequestProgressSnapshot {
        AiRequestProgressSnapshot::default()
    }
}

#[path = "ppt_master_native_density.rs"]
mod native_density;
#[path = "ppt_master_native_design_system.rs"]
mod native_design_system;
...
mod native_theme;
#[path = "ppt_master_native_visual_details.rs"]
mod native_visual_details;
use native_density::{run_space_utilization_check, NativePageDensityContract};
use native_design_system::{
    persist_design_system_spec, validate_deck_design_consistency, NativeDesignSystemSpec,
    NATIVE_DESIGN_SYSTEM_SPEC_FILE,
};
use native_planning::{
...
use native_visual_details::{run_visual_detail_check, NATIVE_VISUAL_DETAIL_CONTRACT};
...
const NATIVE_VISUAL_DETAIL_MAX_AI_ATTEMPTS_PER_PAGE: usize = 2;
...
fn native_intermediate_artifact_paths(project: &Path) -> Vec<String> {
    [
        ...
        project.join(NATIVE_DESIGN_SYSTEM_SPEC_FILE),
        project
            .join("analysis")
            .join(native_design_system::NATIVE_DESIGN_CONSISTENCY_REPORT_FILE),
...
fn native_failure_type(stage: &str, error: &str) -> &'static str {
    ...
    match stage {
        ...
        "validate_visual_details" => "native_visual_detail_validation_failed",
...
fn native_page_validation_error_message(
    ...
    let checker_name = if stage == "validate_visual_details" {
        "原生 SVG 视觉细节检查"
    } else {
        match stage {
            ...
            "validate_design_system_consistency" => "原生 SVG 跨页设计系统一致性检查",
            _ => "原生 SVG 文本几何检查",
        }
    };
    format!(
...
fn native_page_validation_may_retry(stage: &str, attempts: usize) -> bool {
    let limit = match stage {
        ...
        "validate_visual_details" => NATIVE_VISUAL_DETAIL_MAX_AI_ATTEMPTS_PER_PAGE,
        _ => return false,
    };
    attempts < limit
}
...
impl PptMasterService {
    ...
    async fn generate_from_prompt_ppt_master_native_with_project(
        ...
        let mut design_system_spec = NativeDesignSystemSpec::from_inputs(
            &theme_spec,
            &style,
            input.custom_style.as_deref(),
            input.extra_requirements.as_deref(),
        ...
        log_lines.push(format!(
            "[Native Design System] contract={} axes={} gridColumns={} safeMargin={} maxFontFamilies={} pageContracts=pending",
            design_system_spec.contract_name,
            design_system_spec.semantic_style_axes.join(","),
            design_system_spec.grid_system.column_count,
            design_system_spec.grid_system.canvas_safe_margin,
            design_system_spec.typography_system.maximum_font_families
        ));
        log_planning_input(
            &input,
            prompt,
        ...
        let mut design_system_spec_path =
            match persist_design_system_spec(&project, &design_system_spec) {
                Ok(path) => path,
                Err(error) => {
                    native_state.set_status("failed");
                    let _ = write_state_atomic(&project, &native_state);
        ...
        native_stage_success(
            ...
            prepare_started,
            &format!(
                "project={},resumed={},state={},themeSpec={},designSystemSpec={}",
                project.display(),
                resumed,
                project.join(NATIVE_STATE_FILE).display(),
                theme_spec_path.display(),
                design_system_spec_path.display()
            ),
        );
            ...
            && native_planning_artifacts_present(&project)
        {
            ...
            design_system_spec.assign_page_contracts(&plan);
            ...
            let design_spec = build_ppt_master_design_spec(
                &plan,
                &planning_context,
                &style_mapping,
                &theme_spec,
            ...
            let spec_lock =
                build_ppt_master_spec_lock(&plan, &style_mapping, &theme_spec, &design_system_spec);
        ...
        design_system_spec.assign_page_contracts(&plan);
        design_system_spec_path = match persist_design_system_spec(&project, &design_system_spec) {
            Ok(path) => path,
            Err(error) => {
                record_native_pipeline_failure(&project, &mut native_state, "build_design_spec");
                return Ok(native_failure_result(
        ...
        match load_or_create_checkpoint(&project, &input_fingerprint, requested_count) {
            Ok(mut checkpoint) => {
                checkpoint.design_system_spec = Some(design_system_spec.clone());
                if let Err(error) = persist_native_planning_checkpoint(&project, &checkpoint) {
                    record_native_pipeline_failure(
                        &project,
                        &mut native_state,
                        "build_design_spec",
        ...
        native_state.artifacts.design_system_spec_path =
            design_system_spec_path.to_string_lossy().to_string();
        log_lines.push(format!(
            "[Native Design System] persisted={} pageContracts={} checkpointed=true",
            design_system_spec_path.display(),
            design_system_spec.page_contracts.len()
        ));
        ...
        for idx in 0..plan.slides.len() {
            ...
            let mut visual_repair_svg: Option<String> = None;
            let mut visual_must_keep_text: Option<Vec<String>> = None;
            let mut page_validation_retry_counts = HashMap::<String, usize>::new();
            ...
            'page_attempt: loop {
                ...
                log_lines.push(format!(
                    ...
                    } else if visual_repair_svg.is_some() {
                        "repair-visual-details"
                    } else {
                        "generate"
                    },
                ...
                let svg = match generate_ppt_master_driven_slide_svg(
                    ...
                    visual_repair_svg.as_deref(),
                    visual_must_keep_text.as_deref(),
                )
                .await
                {
                    Ok(svg) => svg,
                ...
                if normalization != NativeSvgNormalizationReport::default() {
                    log_lines.push(format!(
                    "[SVG Compatibility] file={} rgbaColorsNormalized={} groupOpacityNormalized={} filtersRemoved={} malformedClosingTagsRepaired={} duplicateLineCoordinatesRepaired={} duplicateIdAttributesRepaired={} duplicateTextAnchorAttributesRepaired={} fallback=false",
                    filename,
                    ...
                    normalization.duplicate_id_attributes_repaired,
                    normalization.duplicate_text_anchor_attributes_repaired
                ));
                }
                ...
                let (mut page_failure, page_quality, page_geometry) =
                    match validate_generated_native_page(
                        ...
                        Ok(value) => value,
                        Err(error) => {
                            let failure_stage = if error.to_string().contains("视觉细节") {
                                "validate_visual_details"
                            } else if error.to_string().contains("空间占用") {
                ...
                if page_failure.is_none() {
                    if let (Some(expected), Some(geometry)) =
                        (visual_must_keep_text.as_ref(), page_geometry.as_ref())
                    {
                        let actual = geometry.visible_texts();
                        if !native_page_relayout_preserved_visible_text(expected, &actual) {
                            page_failure = Some(NativePageValidationFailure {
                                stage: "validate_visual_details".to_string(),
                                violated_rule: "visible_text_changed_during_visual_detail_repair"
                                    .to_string(),
                                checker_summary: format!(
                                    "AI local visual-detail repair changed visible text; expected={}; actual={}",
                                    serde_json::to_string(expected).unwrap_or_default(),
                                    serde_json::to_string(&actual).unwrap_or_default()
                                ),
                                text_geometry: Some(geometry.state()),
                ...
                if let Some(failure) = page_failure {
                    ...
                    let stage_retries = page_validation_retry_counts
                        .get(&failure_stage)
                        .copied()
                        .unwrap_or_default();
                    if native_page_validation_may_retry(&failure_stage, stage_retries) {
                        page_validation_retry_counts
                            .insert(failure_stage.clone(), stage_retries.saturating_add(1));
                        native_state.set_stage("execute_slides");
                        ...
                        page_retry_feedback = Some(if failure_stage == "validate_svgs" {
                        ...
                        } else if failure_stage == "validate_visual_details" {
                            visual_repair_svg =
                                fs::read_to_string(project.join("svg_output").join(&filename)).ok();
                            visual_must_keep_text = page_geometry
                                .as_ref()
                            ...
                            format!(
                                "Page P{:02} has visual-detail geometry failures. {} Repair only the listed relationships. Preserve every visible text string, fact, theme, layout family, and visual hierarchy. Snap connectors to declared anchors, keep lines out of text, restore card spacing and group alignment, and keep all structural elements inside the canvas. Do not use fallback.",
                                slide.page, checker_summary
                            )
                            ...
                            geometry_repair_svg = None;
                            geometry_must_keep_text = None;
                            visual_repair_svg = None;
                            visual_must_keep_text = None;
                            format!(
                        ...
                        if (failure_stage != "validate_text_geometry"
                            ...
                            && (failure_stage != "validate_visual_details"
                                || visual_repair_svg.is_some())
                        {
                            log_lines.push(format!(
                                ...
                                if failure_stage == "validate_text_geometry" {
                                    ...
                                    "repair-visual-details-current-svg-only"
                                } else {
        ...
        let design_consistency_started = native_stage_start(
            &mut log_lines,
            "validate_design_system_consistency",
            &format!(
                "designSystemSpec={},pages={}",
                design_system_spec_path.display(),
                plan.slides.len()
            ),
        );
        let mut design_consistency =
            match validate_deck_design_consistency(&project, &design_system_spec) {
                Ok(report) => report,
                Err(error) => {
                    native_state.set_status("failed");
                    let _ = persist_native_state(&project, &native_state);
        ...
        if !design_consistency.passed {
            log_lines.push(format!(
                "[Native Design Consistency] passed=false issues={} failedPages={} action=redo-failed-page-once fallback=false",
                design_consistency.issues.len(),
                sorted_page_list(&design_consistency.failed_pages().into_iter().collect())
            ));
            for page in design_consistency.failed_pages() {
                let Some(idx) = plan.slides.iter().position(|slide| slide.page == page) else {
                    continue;
                };
                let slide = &plan.slides[idx];
                let filename = svg_filename_for_slide(slide);
                let existing_path = svg_output.join(&filename);
                let existing_svg = match fs::read_to_string(&existing_path) {
                    Ok(svg) => svg,
                    Err(error) => {
                        native_state.set_status("failed");
                        let _ = persist_native_state(&project, &native_state);
                        return Ok(native_failure_result(
                            AppError::Custom(format!(
                                "read page for design-system local redo failed: {} ({error})",
                ...
                let feedback = format!(
                    "Deck-level DesignSystemSpec consistency check failed for P{page:02}. {} Redo only this page once. Keep all facts and the current page role. Execute the immutable page layout family, focal point, density, typography, grid, spacing, palette, line, card, connector, and decoration rules. Make the page visibly different from adjacent layouts without changing its semantic layout. Do not use fallback.",
                    design_consistency.page_summary(page)
                );
                {
                    let page_state = native_state.page_mut(page);
                    page_state.status = "generating".to_string();
                    page_state.attempts = page_state.attempts.saturating_add(1);
                    page_state.updated_at = native_state_now();
                }
                native_state.set_stage("validate_design_system_consistency");
                persist_native_state(&project, &native_state)?;
                let prev_title = idx
                    .checked_sub(1)
                    .and_then(|index| plan.slides.get(index))
                ...
                let next_title = plan
                    ...
                    .unwrap_or("");
                let density_contract = NativePageDensityContract::for_slide(slide);
                let regenerated = generate_ppt_master_driven_slide_svg(
                    db,
                    &skill_text,
                    &resources,
                    &design_spec,
                ...
                let regenerated = match regenerated {
                    ...
                    Err(error) if !block_on_quality_failure => {
                        quality_check_passed = false;
                        log_lines.push(format!(
                            "[Native Design Consistency] page=P{page:02} localRedo=false blockOnQualityFailure=false action=keep-last-svg-and-continue error={} fallback=false",
                            single_line_log_value(&error.to_string())
                        ));
                        continue;
                    }
                ...
                let (regenerated, normalization) = normalize_native_svg_compatibility(&regenerated);
                if normalization != NativeSvgNormalizationReport::default() {
                    log_lines.push(format!(
                        "[SVG Compatibility] file={} designSystemRedo=true rgbaColorsNormalized={} groupOpacityNormalized={} filtersRemoved={} malformedClosingTagsRepaired={} duplicateLineCoordinatesRepaired={} duplicateIdAttributesRepaired={} duplicateTextAnchorAttributesRepaired={} fallback=false",
                        filename,
                        normalization.rgba_colors_normalized,
                        normalization.group_opacity_normalized,
                        normalization.filters_removed,
                ...
                if let Err(error) = validate_native_svg_text(&filename, &regenerated) {
                    if !block_on_quality_failure {
                        quality_check_passed = false;
                        log_lines.push(format!(
                            "[Native Design Consistency] page=P{page:02} localRedoInvalid=true blockOnQualityFailure=false action=restore-last-svg-and-continue error={} fallback=false",
                            single_line_log_value(&error.to_string())
                        ));
                        let _ = write_file(&existing_path, &existing_svg);
                        continue;
                    }
                    native_state.set_status("failed");
                    let _ = persist_native_state(&project, &native_state);
                ...
                if let Err(error) = write_file(&existing_path, &regenerated) {
                    native_state.set_status("failed");
                    let _ = persist_native_state(&project, &native_state);
                    return Ok(native_failure_result(
                        error,
                ...
                let page_validation = validate_generated_native_page(
                    &root,
                    &input.python_path,
                    &project,
                    &filename,
                ...
                match page_validation {
                    Ok((None, _, geometry)) => {
                        if let Some(geometry) = geometry {
                            set_native_text_geometry_state(
                                &mut native_state,
                                page,
                                geometry.state(),
                            );
                        ...
                        persist_native_state(&project, &native_state)?;
                        log_lines.push(format!(
                            "[Native Design Consistency] page=P{page:02} localRedo=true pageValidation=true fallback=false"
                        ));
                    }
                    Ok((Some(failure), _, _)) if !block_on_quality_failure => {
                        quality_check_passed = false;
                        log_lines.push(format!(
                            "[Native Design Consistency] page=P{page:02} localRedo=true pageValidation=false blockOnQualityFailure=false action=keep-last-svg-and-continue rule={} fallback=false",
                            failure.violated_rule
                        ));
                    }
                    Ok((Some(failure), _, _)) => {
                    ...
                    Err(error) if !block_on_quality_failure => {
                        quality_check_passed = false;
                        log_lines.push(format!(
                            "[Native Design Consistency] page=P{page:02} localRedo=true checkerError={} blockOnQualityFailure=false action=keep-last-svg-and-continue fallback=false",
                            single_line_log_value(&error.to_string())
                        ));
                    }
                    Err(error) => {
        ...
        if !design_consistency.passed {
            quality_check_passed = false;
            let first = design_consistency.issues.first().cloned().unwrap_or(
                native_design_system::NativeDesignConsistencyIssue {
                    page_number: None,
                    rule: "design_system_consistency_failed".to_string(),
                    summary: "deck-level design consistency failed".to_string(),
                },
            );
            if block_on_quality_failure {
                native_state.set_status("failed");
                let _ = persist_native_state(&project, &native_state);
                let page = first.page_number;
                let file_name = page
                    .and_then(|page| plan.slides.iter().find(|slide| slide.page == page))
                    .map(svg_filename_for_slide)
                    .unwrap_or_else(|| "deck.svg".to_string());
                let quality_failure = NativeQualityFailure {
                    page_number: page,
                    file_name: file_name.clone(),
                    violated_rule: first.rule,
                    checker_summary: first.summary,
                };
                let result = native_failure_result(
                    AppError::Custom(native_page_validation_error_message(
                        &project,
            ...
            log_lines.push(format!(
                "[Native Design Consistency] passed=false blockOnQualityFailure=false action=keep-last-svg-and-continue issues={} fallback=false",
                design_consistency.issues.len()
            ));
        }
        native_stage_success(
            ...
            design_consistency_started,
            &format!(
                "passed={},pages={},issues={},report={}",
                design_consistency.passed,
                design_consistency.pages.len(),
                design_consistency.issues.len(),
                project
                    .join("analysis")
                    .join(native_design_system::NATIVE_DESIGN_CONSISTENCY_REPORT_FILE)
...
fn build_native_input_fingerprint(
    ...
    design_system_spec: &NativeDesignSystemSpec,
    ...
    let fingerprint_input = NativeFingerprintInput {
        ...
        design_system_spec: design_system_spec.planning_contract(),
...
fn prepare_existing_native_pages(
    ...
    for slide in &plan.slides {
        ...
        if normalized != original {
            ...
            preparation.upstream_changed = true;
            log_lines.push(format!(
                "[Native Resume] page=P{:02} action=mechanical-repair file={} rgbaColorsNormalized={} groupOpacityNormalized={} filtersRemoved={} malformedClosingTagsRepaired={} duplicateLineCoordinatesRepaired={} duplicateIdAttributesRepaired={} duplicateTextAnchorAttributesRepaired={} fallback=false",
                slide.page,
                file_name,
                ...
                report.duplicate_id_attributes_repaired,
                report.duplicate_text_anchor_attributes_repaired
            ));
        }
    ...
    for slide in &plan.slides {
        ...
        let mut geometry = run_native_text_geometry_check(python_path, &geometry_path)?;
        ...
        let visual_report_path = project
            .join("analysis")
            .join("native_visual_details")
            .join(format!("P{:02}.json", slide.page));
        let visual_before = fs::read_to_string(&geometry_path).map_err(|error| {
            AppError::Custom(format!(
                "读取视觉细节检查前 SVG 失败: {} ({error})",
                geometry_path.display()
            ))
        })?;
        let visual = run_visual_detail_check(python_path, &geometry_path, &visual_report_path)?;
        let visual_changed = fs::read_to_string(&geometry_path)
            .map(|current| current != visual_before)
            .unwrap_or(false);
        if visual_changed {
            preparation.upstream_changed = true;
        }
        log_lines.push(format!(
            "[Native Resume] page=P{:02} visualDetailsPassed={} hardErrors={} warnings={} autoFixes={} file={}",
            slide.page,
            visual.passed,
            visual.hard_errors.len(),
            visual.warnings.len(),
            visual.auto_fix_applied.len(),
            file_name
        ));
        if !visual.passed {
            ...
            set_native_page_state(
                ...
                "failed",
                Some(summary.clone()),
                Some(visual.violated_rule()),
                Some(summary),
                false,
            );
            state.current_stage = "validate_visual_details".to_string();
            persist_native_state(project, state)?;
            continue;
        }
        if !visual.auto_fix_applied.is_empty() {
            geometry = run_native_text_geometry_check(python_path, &geometry_path)?;
            set_native_text_geometry_state(state, slide.page, geometry.state());
            if !geometry.passed {
                let summary = format!(
                    "visual detail deterministic repair introduced text geometry errors: {}",
                    geometry.summary()
                );
                set_native_page_state(
                    state,
                    ...
                    "failed",
                    Some(summary.clone()),
                    Some("visual_detail_repair_created_text_collision".to_string()),
                    Some(summary),
                    false,
                );
                state.current_stage = "validate_visual_details".to_string();
...
> {
    ...
    let mut geometry = run_native_text_geometry_check(python_path, &svg_path)?;
    if !geometry.passed {
    ...
    let visual = run_visual_detail_check(python_path, &svg_path, &visual_report_path)?;
    log_lines.push(format!(
        "[Visual Details] file={} passed={} hardErrors={} warnings={} autoFixes={} report={}",
        file_name,
        visual.passed,
        visual.hard_errors.len(),
        visual.warnings.len(),
        visual.auto_fix_applied.len(),
        visual_report_path.display()
    ));
    if !visual.passed {
        return Ok((
            Some(NativePageValidationFailure {
                stage: "validate_visual_details".to_string(),
                violated_rule: visual.violated_rule(),
                checker_summary: visual.summary(),
                text_geometry: Some(geometry.state()),
            }),
            quality,
            Some(geometry),
    ...
    if !visual.auto_fix_applied.is_empty() {
        geometry = run_native_text_geometry_check(python_path, &svg_path)?;
        if !geometry.passed {
            return Ok((
                Some(NativePageValidationFailure {
                    stage: "validate_visual_details".to_string(),
                    violated_rule: "visual_detail_repair_created_text_collision".to_string(),
                    checker_summary: geometry.summary(),
                    text_geometry: Some(geometry.state()),
                }),
                quality,
...
pub(super) async fn generate_native_from_value(
    db: &Database,
    request: serde_json::Value,
) -> Result<serde_json::Value, AppError> {
    let input: PptMasterGenerateInput = serde_json::from_value(request)
        .map_err(|error| AppError::Custom(format!("解析严格 PPT 生成请求失败: {error}")))?;
    let result = PptMasterService::generate_from_prompt_ppt_master_native(db, input).await?;
    serde_json::to_value(result)
        .map_err(|error| AppError::Custom(format!("序列化严格 PPT 生成结果失败: {error}")))
}
...
async fn generate_native_structured_slide_plan(
    ...
    if let Some(existing) = checkpoint.design_system_spec.as_ref() {
        if !existing.base_contract_eq(design_system_spec) {
            return Err(AppError::Custom(
                "native planning checkpoint design system does not match current input fingerprint"
                    .to_string(),
            ));
        }
    } else {
    ...
    let deck_context = build_native_deck_outline_context(
        input,
        title,
        audience,
        slide_count,
...
fn build_native_deck_outline_context(
    ...
    format!(
        "Return JSON only for a DeckOutline. Do not output markdown, SVG, long body copy, coordinates, or full source material.\n\
         ...
         Deck-wide DesignSystemSpec (immutable structure, typography, grid, spacing, graphic language, hierarchy, and rhythm; use it to plan semantic page roles, never output its field names as visible copy):\n{design_system_contract}\n\n\
         Structured material summary:\n{understanding}"
        ,
        theme_contract = theme_spec.prompt_contract(),
        design_system_contract = design_system_spec.planning_contract()
    )
}
...
async fn generate_ppt_master_driven_slide_svg(
    ...
    visual_repair_svg: Option<&str>,
    visual_must_keep_text: Option<&[String]>,
) -> Result<String, AppError> {
    ...
    let page_design_contract = design_system_spec
        .page_contract(slide.page)
        .ok_or_else(|| {
            AppError::Custom(format!(
                "DesignSystemSpec is missing the page contract for P{:02}",
                slide.page
            ))
        })?;
    let page_design_contract_json =
        serde_json::to_string_pretty(page_design_contract).map_err(|error| {
            AppError::Custom(format!("serialize page design contract failed: {error}"))
        })?;
    let previous_layout = slide.page.checked_sub(1).and_then(|page| {
        design_system_spec.page_contract(page).map(|contract| {
            format!(
                "P{page:02}: {} / {}",
                contract.layout_family, contract.focal_point
            )
        })
    });
    let next_layout = design_system_spec
        .page_contract(slide.page + 1)
        .map(|contract| {
            format!(
                "P{:02}: {} / {}",
                contract.page_index, contract.layout_family, contract.focal_point
            )
        });
    let adjacent_layout_summary = format!(
        "previous={}; next={}",
        previous_layout.as_deref().unwrap_or("none"),
        next_layout.as_deref().unwrap_or("none")
    );
    let locked_page_context = format!(
        "Current page execution lock:\n\
         ...
         - dominant_visual_type: {dominant_visual_type}\n\
        ...
        page_rhythm = page_design_contract.page_rhythm,
        layout_family = page_design_contract.layout_family,
        focal_point = page_design_contract.focal_point,
        dominant_visual_type = page_design_contract.dominant_visual_type,
        density_target = page_design_contract.content_density,
        adjacent_layouts = adjacent_layout_summary,
    ...
    let text_geometry_contract = format!(
        "Native text geometry contract:\n\
         ...
         {visual_detail_contract}\n\
         {retry_feedback}",
        visual_detail_contract = NATIVE_VISUAL_DETAIL_CONTRACT,
    ...
    let current_page_task = format!(
        "Current page task contract:\n\
         ...
         [Current page cross-slide design contract]\n{page_design_contract}\n\n\
         [Adjacent page layout summary]\n{adjacent_layout_summary}\n\n\
        ...
        page_design_contract = page_design_contract_json,
        adjacent_layout_summary = adjacent_layout_summary,
    ...
    let full_ai_prompt = format!(
        "你是 ppt-master Executor。请按照 ppt-master 的设计体系逐页手写 SVG，而不是生成简单占位框。\n\
         ...
         9. DesignSystemSpec 是整套页面唯一且不可重新解释的跨页合同；不得自行更换字体、字号层级、网格、边距、间距、圆角、线条、卡片、连接器、装饰或跨页节奏。当前页必须执行已分配的 layout family、focal point、dominant visual 和 density target。\n\n\
         【NativeThemeSpec — deck-wide immutable】\n{theme_contract}\n\n\
         【DesignSystemSpec — deck-wide immutable】\n{design_system_contract}\n\n\
        ...
        theme_contract = theme_spec.prompt_contract(),
        design_system_contract = design_system_spec.prompt_contract(),
    ...
    } else if let Some(current_svg) = visual_repair_svg {
        let slide_spec = spec_lock_path
            .parent()
            .map(|project| {
                project
            ...
            .unwrap_or_else(|| serde_json::to_string_pretty(slide).unwrap_or_default());
        format!(
            "You are performing exactly one page-local SVG visual-detail repair.\n\
             Return only one complete 1280x720 SVG. Do not re-plan the deck and do not use fallback.\n\
             Preserve every visible text string exactly and preserve all facts, theme colors, layout family, focal point, and visual hierarchy.\n\
             Preserve every existing id and every data-pome-* attribute. Every visible text element must retain its complete data-pome text-region contract; never remove or replace those attributes while repairing visual relationships.\n\
             Repair only the reported text/shape/card/connector/alignment relationships. Snap connector endpoints to the declared node or card boundary, route connectors away from text, restore card padding and gaps, align declared groups, and keep structural elements inside the canvas.\n\
             Do not delete, rewrite, duplicate, or add visible content. Do not mechanically replace the page with a card grid.\n\
             Re-check text geometry and every visual-detail contract item before returning.\n\n\
             [NativeThemeSpec — deck-wide immutable]\n{theme_contract}\n\n\
             [DesignSystemSpec — deck-wide immutable]\n{design_system_contract}\n\n\
             [Current page design contract]\n{page_design_contract}\n\n\
             ...
             [Current SlideSpec]\n{slide_spec}\n\n\
             [Must keep visible text]\n{must_keep}\n\n\
             [Measured visual-detail failures]\n{repair_context}\n\n\
             [Current SVG]\n{current_svg}\n",
            theme_contract = theme_spec.prompt_contract(),
            design_system_contract = design_system_spec.prompt_contract(),
            page_design_contract = page_design_contract_json,
            ...
            slide_spec = slide_spec,
            must_keep = serde_json::to_string_pretty(
                &visual_must_keep_text.unwrap_or_default()
            )
            .unwrap_or_default(),
            repair_context = retry_feedback.unwrap_or("{}"),
            current_svg = current_svg,
...
struct NativeSvgNormalizationReport {
    ...
    duplicate_id_attributes_repaired: usize,
    duplicate_text_anchor_attributes_repaired: usize,
}
...
fn normalize_native_svg_compatibility(svg: &str) -> (String, NativeSvgNormalizationReport) {
    ...
    let rgba_normalized = native_opening_tag_regex()
        ...
        .into_owned();
    let (id_normalized, duplicate_id_attributes_repaired) =
        normalize_duplicate_native_id_attributes(&rgba_normalized);
    let (line_coordinate_normalized, duplicate_line_coordinates_repaired) =
        normalize_duplicate_native_line_coordinate_attributes(&id_normalized);
    let (text_anchor_normalized, duplicate_text_anchor_attributes_repaired) =
        normalize_duplicate_native_text_anchor_attributes(&line_coordinate_normalized);
    let mut normalized_count = 0;
    let group_normalized = native_group_opacity_regex()
        .replace_all(&text_anchor_normalized, |captures: &regex::Captures<'_>| {
            normalized_count += 1;
    ...
    (
        ...
        NativeSvgNormalizationReport {
            ...
            duplicate_id_attributes_repaired,
            duplicate_text_anchor_attributes_repaired,
        },
    )
}

fn normalize_duplicate_native_id_attributes(svg: &str) -> (String, usize) {
    let referenced_ids = native_pome_reference_attribute_regex()
        .captures_iter(svg)
        .filter_map(|capture| {
            capture
                .name("value")
        ...
        .collect::<HashSet<_>>();
    let mut repaired = 0;
    let normalized = native_opening_tag_regex()
        .replace_all(svg, |captures: &regex::Captures<'_>| {
            let tag = captures.get(0).map_or("", |value| value.as_str());
            let ids = native_id_attribute_regex()
                .captures_iter(tag)
                .filter_map(|attribute| {
                    Some((
                        attribute.get(0)?.range(),
                        attribute.name("value")?.as_str().to_string(),
                    ))
                })
                .collect::<Vec<_>>();
            if ids.len() != 2 {
            ...
            let keep = if ids[0].1 == ids[1].1 {
                Some(0)
            } else {
                match (
                    referenced_ids.contains(&ids[0].1),
                    referenced_ids.contains(&ids[1].1),
                ) {
                    (true, false) => Some(0),
                    (false, true) => Some(1),
                    _ => None,
            ...
            result.replace_range(ids[remove].0.clone(), "");
            repaired += 1;
            result
        })
        .into_owned();
...
fn normalize_duplicate_native_text_anchor_attributes(svg: &str) -> (String, usize) {
    let mut repaired = 0;
    let normalized = native_text_opening_tag_regex()
        .replace_all(svg, |captures: &regex::Captures<'_>| {
            let tag = captures.get(0).map_or("", |value| value.as_str());
            let anchors = native_text_anchor_attribute_regex()
                .captures_iter(tag)
                .filter_map(|attribute| {
                    Some((
                        attribute.get(0)?.range(),
                        attribute.name("value")?.as_str().to_ascii_lowercase(),
                    ))
                })
                .collect::<Vec<_>>();
            if anchors.len() != 2 {
            ...
            let keep = if anchors[0].1 == anchors[1].1 {
                Some(0)
            } else {
                let numeric = native_text_region_numeric_attribute_regex()
                    .captures_iter(tag)
                    .filter_map(|attribute| {
                        Some((
                            attribute.name("name")?.as_str().to_ascii_lowercase(),
                            attribute.name("value")?.as_str().parse::<f64>().ok()?,
                        ))
                    })
                    .collect::<HashMap<_, _>>();
                let x = numeric.get("x").copied();
                let region_x = numeric.get("data-pome-region-x").copied();
                let region_width = numeric.get("data-pome-region-width").copied();
                match (x, region_x, region_width) {
                    (Some(x), Some(region_x), Some(width)) if width > 0.0 => {
                        let relative = (x - region_x) / width;
                        let expected = if relative <= 1.0 / 3.0 {
                            "start"
                        } else if relative >= 2.0 / 3.0 {
                            "end"
                        } else {
                            "middle"
                        };
                        anchors.iter().position(|(_, value)| value == expected)
                    }
                    _ => None,
                }
            };
            ...
            let remove = 1 - keep;
            let mut result = tag.to_string();
            result.replace_range(anchors[remove].0.clone(), "");
            repaired += 1;
            result
        })
        .into_owned();
...
fn native_id_attribute_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r#"(?i)\s+id\s*=\s*(?P<quote>[\"'])(?P<value>[^\"']*)[\"']"#)
            .expect("valid regex")
    })
}

fn native_pome_reference_attribute_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(
            r#"(?i)data-pome-(?:owner|from|to)\s*=\s*(?P<quote>[\"'])(?P<value>[^\"']*)[\"']"#,
        )
        .expect("valid regex")
    })
}
...
fn native_text_opening_tag_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r#"(?is)<text\b[^>]*>"#).expect("valid regex"))
}

fn native_text_anchor_attribute_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r#"(?i)\s+text-anchor\s*=\s*(?P<quote>[\"'])(?P<value>start|middle|end)[\"']"#)
            .expect("valid regex")
    })
}

fn native_text_region_numeric_attribute_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(
            r#"(?i)(?P<name>data-pome-region-x|data-pome-region-width|x)\s*=\s*(?P<quote>[\"'])(?P<value>-?[0-9]+(?:\.[0-9]+)?)[\"']"#,
        )
        .expect("valid regex")
    })
}
...
fn build_ppt_master_design_spec(
    ...
    out.push_str("## Deck-wide DesignSystemSpec\n\n```json\n");
    out.push_str(&design_system_spec.prompt_contract());
    out.push_str("\n```\n\nThis is the immutable cross-slide execution contract. Page structures may vary, but its palette, typography, grid, spacing, graphic language, hierarchy, and rhythm rules may not be reinterpreted per page. Never render contract fields as visible slide content.\n\n");
    ...
    out.push_str(&format!(
        "| **Content grid** | {} columns, {}px gutter |\n\n",
        design_system_spec.grid_system.column_count, design_system_spec.grid_system.gutter
    ));
    ...
    out.push_str(&format!(
        "- Title: `{}`; size {}-{}\n",
        design_system_spec.typography_system.title_font_family,
        design_system_spec
            .typography_system
            .title_size_range
            .minimum,
    ...
    out.push_str(&format!(
        "- Body: `{}`; size {}-{}\n",
        design_system_spec.typography_system.body_font_family,
        design_system_spec.typography_system.body_size_range.minimum,
        design_system_spec.typography_system.body_size_range.maximum
    ));
    out.push_str(&format!(
        "- Display: `{}`; maximum font families={}\n\n",
        design_system_spec.typography_system.display_font_family,
        design_system_spec.typography_system.maximum_font_families
    ));
...
fn build_ppt_master_spec_lock(
    ...
    out.push_str(&theme_spec.prompt_contract());
    out.push_str("\n```\n\n");
    out.push_str("## native_design_system_spec\n```json\n");
    out.push_str(&design_system_spec.prompt_contract());
    out.push_str("\n```\n\n");
    out.push_str("## colors\n");
    ...
    out.push_str("## native_typography_constraints\n");
    out.push_str(&format!(
        "- maximum_font_families: {}\n- title_range: {}-{}\n- subtitle_range: {}-{}\n- body_range: {}-{}\n- label_range: {}-{}\n- number_range: {}-{}\n\n",
        design_system_spec.typography_system.maximum_font_families,
        design_system_spec.typography_system.title_size_range.minimum,
        design_system_spec.typography_system.title_size_range.maximum,
        design_system_spec.typography_system.subtitle_size_range.minimum,
        design_system_spec.typography_system.subtitle_size_range.maximum,
        design_system_spec.typography_system.body_size_range.minimum,
        design_system_spec.typography_system.body_size_range.maximum,
        design_system_spec.typography_system.label_size_range.minimum,
        design_system_spec.typography_system.label_size_range.maximum,
        design_system_spec.typography_system.number_size_range.minimum,
        design_system_spec.typography_system.number_size_range.maximum
    ));
...
mod native_theme_contract_tests {
    ...
    fn red_custom_style_does_not_fall_back_to_swiss_blue_orange_lock() {
        ...
        let design_system = NativeDesignSystemSpec::from_inputs(
            &theme,
            "红色情怀",
            input.custom_style.as_deref(),
            input.extra_requirements.as_deref(),
        ...
        let lock = build_ppt_master_spec_lock(&plan, &mapping, &theme, &design_system);
    ...
    fn tech_blue_mapping_and_lock_remain_dark_tech() {
        ...
        let design_system = NativeDesignSystemSpec::from_inputs(&theme, "科技蓝", None, None, None);
        let lock =
            build_ppt_master_spec_lock(&minimal_plan("科技蓝"), &mapping, &theme, &design_system);
    ...
    #[test]
    fn native_spec_lock_typography_uses_numeric_px_slots() {
        let root = std::env::temp_dir().join(format!(
            "pome-native-spec-lock-typography-{}",
            std::process::id()
        ));
        let input: PptMasterGenerateInput = serde_json::from_value(serde_json::json!({
            "pptMasterRoot": root,
            "pythonPath": "python",
            "prompt": "topic",
            "style": "technology-blue"
        }))
        .expect("deserialize native input");
        let theme = NativeThemeSpec::from_inputs("technology-blue", None, None, None);
        let mapping = resolve_style_mapping(&root, "technology-blue", &input, &theme);
        let design_system =
            NativeDesignSystemSpec::from_inputs(&theme, "technology-blue", None, None, None);
        let lock = build_ppt_master_spec_lock(
            &minimal_plan("technology-blue"),
            &mapping,
            &theme,
            &design_system,
        );
        ...
        for line in typography.lines().filter(|line| line.starts_with("- ")) {
            let (key, value) = line
                .trim_start_matches("- ")
                .split_once(": ")
                .expect("typography data row");
            if key == "font_family" || key.ends_with("_family") {
                continue;
            }
            assert!(
                value.parse::<f64>().is_ok(),
                "typography size must be a unitless number: {line}"
            );
        }
        assert!(!typography.contains("_range:"));
        assert!(!typography.contains("maximum_font_families"));
        assert!(lock.contains("## native_typography_constraints\n"));
        assert!(lock.contains("- title_range: 32-76"));
    }
}
...
mod native_strict_pipeline_tests {
    ...
    #[test]
    fn native_compatibility_repairs_repeated_tspan_suffix() {
        let source = r#"<svg viewBox="0 0 1280 720" width="1280" height="720"><text><tspan>content</tspanspan></text></svg>"#;
        let (normalized, report) = normalize_native_svg_compatibility(source);

        assert_eq!(report.malformed_closing_tags_repaired, 1);
        assert_eq!(
            normalized,
            r#"<svg viewBox="0 0 1280 720" width="1280" height="720"><text><tspan>content</tspan></text></svg>"#
        );
        validate_native_svg_text("01_tspan.svg", &normalized).unwrap();
    }
    ...
    #[test]
    fn native_compatibility_keeps_referenced_duplicate_id_value() {
        let source = r#"<svg viewBox="0 0 1280 720" width="1280" height="720"><g id="portrait-region" data-pome-visual-role="card" id="card-portrait"><text data-pome-owner="card-portrait">人物</text></g></svg>"#;

        let (normalized, report) = normalize_native_svg_compatibility(source);

        assert_eq!(report.duplicate_id_attributes_repaired, 1);
        assert!(!normalized.contains("portrait-region"));
        assert_eq!(normalized.matches("id=\"").count(), 1);
        assert!(normalized.contains("id=\"card-portrait\""));
        validate_native_svg_text("01_duplicate_id.svg", &normalized).unwrap();
    }

    #[test]
    fn native_compatibility_does_not_guess_between_unreferenced_duplicate_ids() {
        let source = r#"<svg viewBox="0 0 1280 720" width="1280" height="720"><g id="portrait-region" id="card-portrait"/></svg>"#;

        let (normalized, report) = normalize_native_svg_compatibility(source);

        assert_eq!(report.duplicate_id_attributes_repaired, 0);
        assert_eq!(normalized, source);
        assert_eq!(normalized.matches(" id=\"").count(), 2);
    }
    ...
    #[test]
    fn native_compatibility_repairs_duplicate_text_anchor_from_region_contract() {
        let source = r#"<svg viewBox="0 0 1280 720" width="1280" height="720"><text text-anchor="start" data-pome-region-x="680" data-pome-region-width="540" x="700" text-anchor="end">标题</text></svg>"#;
        let (normalized, report) = normalize_native_svg_compatibility(source);

        assert_eq!(report.duplicate_text_anchor_attributes_repaired, 1);
        assert_eq!(normalized.matches("text-anchor=").count(), 1);
        assert!(normalized.contains("text-anchor=\"start\""));
        assert!(!normalized.contains("text-anchor=\"end\""));
    }

    #[test]
    fn native_compatibility_keeps_duplicate_text_anchor_without_region_contract() {
        let source = r#"<svg viewBox="0 0 1280 720" width="1280" height="720"><text text-anchor="start" x="700" text-anchor="end">标题</text></svg>"#;
        let (normalized, report) = normalize_native_svg_compatibility(source);

        assert_eq!(report.duplicate_text_anchor_attributes_repaired, 0);
        assert_eq!(normalized, source);
    }
...
"#;
    ...
    #[test]
    fn native_visual_detail_contract_requires_structural_metadata_without_theme_rules() {
        assert!(NATIVE_VISUAL_DETAIL_CONTRACT.contains("data-pome-visual-role"));
        assert!(NATIVE_VISUAL_DETAIL_CONTRACT.contains("data-pome-from"));
        assert!(NATIVE_VISUAL_DETAIL_CONTRACT.contains("data-pome-to"));
        assert!(NATIVE_VISUAL_DETAIL_CONTRACT.contains("data-pome-align-group"));
        assert!(!NATIVE_VISUAL_DETAIL_CONTRACT.contains("科技蓝"));
        assert!(!NATIVE_VISUAL_DETAIL_CONTRACT.contains("红色"));
    }
...
mod native_real_entry_tests {
    ...
    async fn native_debug_loop_from_snapshot() {
        ...
        let design_system_spec = NativeDesignSystemSpec::from_inputs(
            &theme_spec,
            style,
            snapshot.payload.custom_style.as_deref(),
            snapshot.payload.extra_requirements.as_deref(),
