...
use crate::services::document_sources::DocumentSource;
...
impl LocalLearningPlanService {
    pub fn calculate(
        ...
        for item in input.selected_learning_sources {
            let source = sources
                .iter()
                .find(|source| source.id == item.document_source_id)
                .ok_or_else(|| format!("文档 ID {} 不存在。", item.document_source_id))?;
            if !source.is_enabled || !source.is_available {
                return Err(format!("文档“{}”当前不可用。", source.display_name));
            }
        ...
        {
            return Err("请至少将一个数据文件设置为“常规资料”“重点资料”或“核心资料”。“仅供参考”文件不会自动加入学习计划。".into());
        }
        ...
        let summary = LearningTimeSummary {
            ...
            target_coverage_rate: planned / target,
        ...
        if summary.missing_hours > 0.0 {
            warnings.push(format!("当前可用时间为{}小时，低于当前学习目标建议的{}小时。本计划将优先安排权重较高的数据。",summary.available_hours,summary.target_hours));
        }
...
fn allocate_half_hour_blocks(total: f64, weights: &[f64]) -> Result<Vec<f64>, String> {
    ...
    for index in order.into_iter().take(blocks - assigned) {
        allocated[index] += 1;
    }
    Ok(allocated
        .into_iter()
...
pub fn combined_learning_weight(source_weight: f64, knowledge_weight: f64) -> f64 {
    source_weight * knowledge_weight
}

#[cfg(test)]
mod tests {
    use super::*;
    fn source(id: i64) -> DocumentSource {
        DocumentSource {
            id,
            display_name: format!("{id}.xlsx"),
            original_file_name: format!("{id}.xlsx"),
            stored_relative_path: format!("dev-document-data/{id}.xlsx"),
            file_extension: "xlsx".into(),
            mime_type: String::new(),
            category: "本地数据".into(),
            source_module: "learning-assistant".into(),
            is_builtin: true,
            is_enabled: true,
            file_size: 1,
            checksum: String::new(),
    ...
    ) -> (LocalLearningPlanInput, Vec<DocumentSource>) {
        (
    ...
    fn capacity_examples() {
        ...
        let r = LocalLearningPlanService::calculate(i, s).unwrap();
        assert_eq!(
            (
                r.time_summary.target_hours,
                r.time_summary.available_hours,
                r.time_summary.planned_hours,
                r.time_summary.missing_hours
            ),
            (48.0, 21.0, 21.0, 27.0)
        );
        let (i, s) = make(
        ...
        let r = LocalLearningPlanService::calculate(i, s).unwrap();
        assert_eq!(r.time_summary.extra_available_hours, 20.0);
    }
    #[test]
    fn allocations_preserve_totals() {
        ...
        let r = LocalLearningPlanService::calculate(i, s).unwrap();
        assert_eq!(
            r.source_allocations
                .iter()
                .map(|x| x.allocated_hours)
                .sum::<f64>(),
            64.0
        );
        assert_eq!(r.source_allocations[3].allocated_hours, 0.0);
        assert_eq!(
            r.stage_allocations
                .iter()
                .map(|x| x.allocated_hours)
        ...
        assert!(r.daily_allocations.iter().all(|x| x.planned_hours <= 3.0));
    }
    #[test]
    fn all_reference_rejected() {
        ...
        assert!(LocalLearningPlanService::calculate(i, s).is_err());
    }
    #[test]
    fn knowledge_weight_fallback() {
        ...
        assert!((combined_learning_weight(1.5, 1.3) - 1.95).abs() < 1e-9);
    }
}
