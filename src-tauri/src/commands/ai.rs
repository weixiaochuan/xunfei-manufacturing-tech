...
pub async fn test_ai_model(
        ...
        .unwrap_or(true)
    {
        if let Some(credential_id) = input.credential_id.as_deref() {
            input.api_key = crate::services::credentials::CredentialService::load_api_key(
                &state.db,
                state.db.data_dir(),
                credential_id,
            )
...
pub async fn send_ai_message(
    ...
    conversation_id: i64,
    message: String,
    effective_message: Option<String>,
    plugin_system_context: Option<String>,
    use_rag: Option<bool>,
    ...
    let visible_message = crate::services::ai::build_message_with_attachments(
        &message,
        attachments.as_deref().unwrap_or(&[]),
    );
    let effective_message = effective_message
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or(&message);
    let model_message = crate::services::ai::build_message_with_attachments(
        effective_message,
        attachments.as_deref().unwrap_or(&[]),
    );
    let planning_system_context = PlanningService::planning_system_message(
        &state.db,
        &state.data_dir,
    ...
    .map(|context| {
        context.map(|text| {
            format!(
                "{}\n\n{}",
                text.replace("planningUpdate", "planning_update tool"),
    ...
    let result = if use_skills {
        AiService::chat_stream_with_skills(
            app.clone(),
            db,
            &state.data_dir,
            conversation_id,
            &visible_message,
            &model_message,
            planning_system_context.as_deref(),
            plugin_system_context.as_deref(),
            cancel_rx,
        )
        .await
    } else {
    ...
    let planning_status = match &result {
        Ok(_) => "completed",
        Err(err) if err.to_string().contains("cancel") || err.to_string().contains("取消") => {
            "cancelled"
        }
        Err(_) => "failed",
    };
    let assistant_output = if planning_status == "completed" {
        state
            .db
            .list_ai_messages(conversation_id)
            .ok()
    ...
    .ok();
    if planning_status == "completed" && !assistant_output.is_empty() {
        let visible = PlanningService::sanitize_visible_response(&assistant_output);
        if visible != assistant_output {
            if let Ok(conn) = state.db.conn_lock() {
                let _ = conn.execute(
                    "UPDATE ai_messages
                     ...
                     AND content = ?3",
                    rusqlite::params![conversation_id, visible, assistant_output],
                );
            }
        }
    }
...
#[tauri::command]
pub fn finalize_ai_plugin_output(
    state: State<'_, AppState>,
    conversation_id: i64,
    ...
    if final_output.trim().is_empty() {
        return Err("插件后处理结果不能为空".to_string());
    }
    if final_output.chars().count() > MAX_PLUGIN_OUTPUT_CHARS {
        return Err("插件后处理结果超过允许大小".to_string());
    }
    ...
    let updated = state
        .db
        .finalize_ai_plugin_output(conversation_id, &expected_output, &final_output)
        .map_err(|e| e.to_string())?;
    if !updated {
        return Err("AI 回复已经变化，已拒绝写入过期的插件处理结果".to_string());
    }
    Ok(())
}
