...
/// 递归扫描可导入的混合文档，按每个文件自身类型识别。
#[tauri::command]
pub fn scan_supported_import_folder(
    state: tauri::State<'_, AppState>,
    path: String,
) -> Result<Vec<ScannedFile>, String> {
    services::import::ImportService::scan_supported_folder(&state.db, &path)
        .map_err(|e| e.to_string())
}
...
/// 批量导入 Markdown、TXT、PDF、Word 和 Excel；支持混合选择及文件夹层级恢复。
#[tauri::command]
pub async fn import_mixed_files(
    state: tauri::State<'_, AppState>,
    app: AppHandle,
    file_paths: Vec<String>,
    folder_id: Option<i64>,
    ...
    services::import::ImportService::import_mixed_files(
        &state.db,
        &file_paths,
        folder_id,
        root_path.as_deref(),
