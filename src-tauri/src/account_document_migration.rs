...
fn read_snapshot(database_path: &Path) -> Result<LocalSnapshot, String> {
        ...
        .map_err(|_| migration_error("无法读取 SQLite 文档"))?
    {
        let id: i64 = row
            .get(0)
            .map_err(|_| migration_error("SQLite 文档 ID 无效"))?;
        let title: String = row
            .get(1)
            .map_err(|_| migration_error("SQLite 文档标题无效"))?;
        let content: String = row
            .get(2)
            .map_err(|_| migration_error("SQLite 文档正文无效"))?;
        let folder_id: Option<i64> = row
            .get(3)
            .map_err(|_| migration_error("SQLite 文件夹字段无效"))?;
        let is_encrypted: i64 = row
            .get(18)
            .map_err(|_| migration_error("SQLite 加密字段无效"))?;
        if is_encrypted != 0 {
            return Err(migration_error("存在加密文档，安全起见未执行任何导入"));
        }
        let created_at = local_timestamp(
            &row.get::<_, String>(6)
                .map_err(|_| migration_error("SQLite 创建时间无效"))?,
        )?;
        let updated_at = local_timestamp(
            &row.get::<_, String>(7)
                .map_err(|_| migration_error("SQLite 更新时间无效"))?,
        )?;
        let deleted_at = optional_timestamp(
            row.get(10)
                .map_err(|_| migration_error("SQLite 删除时间无效"))?,
        )?;
        let source_path: Option<String> = row
            .get(12)
            .map_err(|_| migration_error("SQLite 来源字段无效"))?;
        let safe_source_name = source_path
            .as_ref()
        ...
        documents.push(ImportDocument {
            source_local_document_id: source_id.clone(),
            title: title.clone(),
    ...
    let tags = tag_statement
        .query_map([], |row| {
            let id = row.get::<_, i64>(0)?;
            Ok(ImportTag {
                source_local_tag_id: format!("sqlite-tag:{id}"),
                name: row.get(1)?,
            })
        })
        .map_err(|_| migration_error("无法读取 SQLite 标签目录"))?
...
fn create_backup(database_path: &Path, backup_directory: &Path) -> Result<PathBuf, String> {
    fs::create_dir_all(backup_directory)
        .map_err(|_| migration_error("无法创建 SQLite 备份目录"))?;
    let stamp = Utc::now().format("%Y%m%dT%H%M%SZ");
    let backup_path = backup_directory.join(format!(
        "markdown-before-import-{stamp}-{}.db",
        Uuid::new_v4()
    ));
    let connection = open_read_only(database_path)?;
    connection
    ...
    if !backup_path.is_file() {
        return Err(migration_error("SQLite 备份文件未生成"));
    }
    Ok(backup_path)
}
...
async fn run_migration(
    ...
    if before.documents.len() > MAX_IMPORT_DOCUMENTS {
        return Err(migration_error("本地文档超过单批导入上限"));
    }
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|_| migration_error("无法创建迁移网络客户端"))?;
    let response = client
        .post(document_migration_server_url(IMPORT_PATH))
        .header(
            AUTHORIZATION,
            document_migration_authorization_header(&token)?,
        )
        .json(&json!({
        ...
        .send()
        .await
        .map_err(|_| migration_error("Account Server 暂不可用"))?;
    if !response.status().is_success() {
        return Err(migration_error("Account Server 拒绝导入"));
    }
    let imported: ImportResponse = response
        .json()
        .await
        .map_err(|_| migration_error("导入响应无效"))?;
    if imported.status != "ok"
        || imported.failed != 0
        || imported.outcomes.len() != before.digests.len()
    {
    ...
    let expected = before
        .digests
        .iter()
        .map(|item| (item.source_id.as_str(), item.content_sha256.as_str()))
        .collect::<BTreeMap<_, _>>();
    let hashes_match = imported.outcomes.iter().all(|item| {
        !item.document_id.is_empty()
            && expected
                .get(item.source_local_document_id.as_str())
                .is_some_and(|hash| *hash == item.content_sha256)
    });
    if !hashes_match {
        return Err(migration_error("导入后正文摘要不一致"));
    }
    let list_response = client
        .get(document_migration_server_url(DOCUMENTS_PATH))
        .header(
            AUTHORIZATION,
            document_migration_authorization_header(&token)?,
        )
        .send()
        .await
        .map_err(|_| migration_error("无法核对服务端文档目录"))?;
    if !list_response.status().is_success() {
        return Err(migration_error("服务端文档目录核对失败"));
    }
    let listed: DocumentListResponse = list_response
        .json()
        .await
        .map_err(|_| migration_error("服务端文档目录响应无效"))?;
    if listed.status != "ok" {
        return Err(migration_error("服务端文档目录响应无效"));
    }
    let server_active_markdown = listed
        .documents
        .iter()
        .filter(|item| {
            item.kind == "markdown"
                ...
                && !item.id.is_empty()
                && !item.title.is_empty()
                && item.markdown_content.is_some()
                && !item.created_at.is_empty()
                && !item.updated_at.is_empty()
        })
        .count();
    let after = read_snapshot(database_path)?;
    let source_database_unchanged = before.database_size == after.database_size
        && before.aggregate_sha256 == after.aggregate_sha256
        && before.documents.len() == after.documents.len();
    if !source_database_unchanged {
        return Err(migration_error("SQLite 原始数据发生变化，已停止验收"));
    }
...
pub async fn account_import_local_markdown_documents(
    ...
    let database_path = app_state.data_dir.join(if cfg!(debug_assertions) {
        "dev-app.db"
    } else {
        "app.db"
    });
...
mod tests {
    ...
    fn timestamp_conversion_uses_explicit_local_offset() {
        assert_eq!(
            local_timestamp("2026-07-22 12:34:56").unwrap(),
    ...
    async fn real_dev_database_import_runner() {
        ...
        println!(
            "{}",
            serde_json::to_string(&report).expect("safe report serialization")
        );
    }
    ...
    async fn real_uploaded_file_document_linkage_runner() {
        ...
        let upload = client
            ...
            .header(
                AUTHORIZATION,
                document_migration_authorization_header(&token).unwrap(),
            )
        ...
        let listed: DocumentListResponse = client
            .get(document_migration_server_url(
        ...
        assert!(listed
            .documents
            .iter()
            .any(|document| document.kind == "uploaded_file"
                && document
                    .file
                    .as_ref()
        ...
        let deleted = client
            ...
            .send()
            .await
            .expect("file delete request failed");
        ...
        let after: DocumentListResponse = client
            ...
            .send()
            .await
            .expect("post-delete document list failed")
            .json()
            .await
            .expect("post-delete document list response");
        assert!(!after.documents.iter().any(|document| document
            .file
            .as_ref()
            .is_some_and(|file| file.id == file_id)));
    ...
    async fn real_unified_document_entry_runner() {
        ...
        let legacy_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../services/account-server/.data/user-files");
        ...
        let internal = create_markdown(format!("entry-new-{marker}"), String::new())
            .await
            .expect("internal Markdown create request failed");
        ...
        assert_eq!(
            internal.pointer("/document/kind").and_then(Value::as_str),
            Some("markdown")
        );
        assert_eq!(
            internal
                .pointer("/document/revision")
        ...
        let upload = client
            ...
            .send()
            .await
            .expect("Markdown upload request failed");
        assert_eq!(upload.status(), reqwest::StatusCode::CREATED);
        ...
        let added = storage_after_upload
            .difference(&storage_before)
            .collect::<Vec<_>>();
        assert_eq!(added.len(), 1);
        assert_eq!(
            std::fs::read(added[0]).expect("stored upload must be readable"),
            source_bytes
        );
        ...
        .await
        .expect("editable Markdown create request failed");
        ...
        assert_eq!(
            imported.pointer("/document/kind").and_then(Value::as_str),
            Some("markdown")
        );
        assert_eq!(
            imported
        ...
        for document_id in [&internal_id, &imported_id] {
            let deleted = client
                .delete(document_migration_server_url(&format!(
                    "/documents/{document_id}"
                )))
                .header(AUTHORIZATION, &authorization)
                .send()
                .await
                .expect("Markdown cleanup failed");
            assert!(deleted.status().is_success());
        }
        let deleted_file = client
            ...
            .send()
            .await
            .expect("uploaded file cleanup failed");
    ...
    async fn real_document_revision_conflict_runner() {
        ...
        let create_response = client
            ...
            .send()
            .await
            .expect("document create request failed");
        assert_eq!(create_response.status(), reqwest::StatusCode::CREATED);
        let created: Value = create_response
            .json()
            .await
            .expect("document create response");
        ...
        let stale_update = client
            .patch(document_migration_server_url(&format!(
                "/documents/{document_id}"
            )))
            .header(AUTHORIZATION, &authorization)
            .json(
                &json!({ "expectedRevision": 1, "markdownContent": "stale content must not win" }),
            )
            .send()
            .await
            .expect("stale update request failed");
        ...
        assert_eq!(
            stale_payload.get("error").and_then(Value::as_str),
        ...
        let saved_content = listed
            ...
            .and_then(|document| document.get("markdownContent"))
            .and_then(Value::as_str);
        ...
        assert!(deleted.status().is_success());
        let restored = client
            .post(document_migration_server_url(&format!(
        ...
        let tag_deleted = client
            ...
            .send()
            .await
            .expect("tag delete failed");
        assert!(tag_deleted.status().is_success());
        let folder_deleted = client
            .delete(document_migration_server_url(&format!(
