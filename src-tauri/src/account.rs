...
use crate::services::data_dir::DataDirResolver;
...
const CREDENTIAL_SERVICE_PREFIX: &str = "cn.edu.pomegranate.account";
const CREDENTIAL_USERNAME_PREFIX: &str = "desktop-session";
...
impl WindowsCredentialStore {
    fn entry() -> Result<keyring::Entry, CredentialError> {
        let (service, username) = credential_slot();
        keyring::Entry::new(&service, &username).map_err(|_| CredentialError)
    }
}

fn credential_slot() -> (String, String) {
    credential_slot_for(
        crate::account_network::deployment_profile(),
        crate::account_network::account_server_origin(),
    )
}

fn credential_slot_for(profile: &str, origin: &str) -> (String, String) {
    let profile = sanitize_credential_segment(profile);
    let origin_hash = hash_credential_segment(origin);
    (
        format!("{CREDENTIAL_SERVICE_PREFIX}.{profile}"),
        format!("{CREDENTIAL_USERNAME_PREFIX}.{origin_hash}"),
    )
}

fn sanitize_credential_segment(value: &str) -> String {
...
fn hash_credential_segment(value: &str) -> String {
...
fn account_success_result(
    app: &AppHandle,
    runtime_state: &AppState,
    user: AccountUser,
) -> AccountLoginResult {
    let restart_required = match crate::framework_app_data_dir(app) {
        Ok(framework_dir) => {
            match DataDirResolver::set_active_account_scope(
                &framework_dir,
                &user.platform_user_id,
                &user.account_number,
                &user.username,
            )
            .and_then(|_| DataDirResolver::account_data_dir(&framework_dir, &user.platform_user_id))
            {
                Ok(expected_dir) => !same_data_dir(&runtime_state.data_dir, &expected_dir),
                Err(error) => {
                    log::warn!("failed to prepare account-scoped data dir: {}", error);
                    false
                }
            }
        }
        Err(error) => {
            log::warn!(
                "failed to locate framework app data dir for account scope: {}",
                error
            );
            false
        }
...
fn same_data_dir(left: &Path, right: &Path) -> bool {
    if left == right {
        return true;
    }
...
async fn restore_session_inner(state: &AccountState) -> AccountLoginResult {
    ...
    match state.remote.get_session(&token).await {
        Ok(user) => AccountLoginResult::Success {
            user,
    ...
    tauri::async_runtime::spawn(async move {
        ...
        match state.remote.exchange_ticket(&ticket).await {
            Ok((token, user)) => {
                ...
                publish_result(&app, account_success_result(&app, &runtime_state, user));
            }
            Err(
                RemoteError::ServiceUnavailable
                | RemoteError::InvalidFolderKind
...
pub async fn restore_account_session(
    app: AppHandle,
    runtime_state: tauri::State<'_, AppState>,
    state: tauri::State<'_, AccountState>,
) -> Result<AccountLoginResult, String> {
    let result = restore_session_inner(&state).await;
    Ok(match result {
        AccountLoginResult::Success { user, .. } => {
            account_success_result(&app, &runtime_state, user)
        }
        other => other,
    })
}
...
pub async fn logout_account(
    ...
    if matches!(result, AccountLoginResult::SignedOut) {
        if let Ok(framework_dir) = crate::framework_app_data_dir(&app) {
            let _ = DataDirResolver::clear_active_account_scope(&framework_dir);
        }
    }
    Ok(result)
}
...
mod tests {
    ...
    fn credential_wrapper_saves_reads_and_deletes_without_serializing_the_token() {
        ...
        let serialized = serde_json::to_string(&AccountLoginResult::Success {
            user: test_user(),
            restart_required: false,
        })
        .unwrap();
    ...
    #[test]
    fn credential_slots_are_partitioned_by_profile_and_account_server() {
        let local = credential_slot_for("local", "http://127.0.0.1:18080");
        let public_test = credential_slot_for("public-ip-test", "http://82.157.119.201:8080");
        let cloud = credential_slot_for("cloud", "https://api.stargathering.cn");

        assert_ne!(local, public_test);
        assert_ne!(public_test, cloud);
        assert_eq!(local.0, "cn.edu.pomegranate.account.local");
        assert_eq!(public_test.0, "cn.edu.pomegranate.account.public-ip-test");
        assert!(local.1.starts_with("desktop-session."));
        assert!(!local.1.contains("127.0.0.1"));
        assert!(!public_test.1.contains("82.157.119.201"));
    }
