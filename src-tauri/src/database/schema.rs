...
#[cfg(test)]
mod marketplace_v3_migration_tests {
    use super::*;

    #[test]
    fn v51_marketplace_versions_upgrade_without_losing_legacy_rows() {
        ...
        conn.execute_batch(
            ...
            INSERT INTO products (id, plugin_id) VALUES ('legacy-product', 'legacy-plugin');
            INSERT INTO product_versions (product_id, version, review_status)
            VALUES ('legacy-product', '1.0.0', 'published');
            "#,
        )
        .unwrap();
        set_version(&conn, 51).unwrap();

        migrate(&conn).unwrap();

        assert_eq!(get_version(&conn).unwrap(), 53);
        let row: (String, i32, String, i32) = conn
            .query_row(
                "SELECT package_format, manifest_schema_version, plugin_id, package_locked
                 FROM product_versions WHERE product_id = 'legacy-product'",
                [],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
            )
            .unwrap();
        assert_eq!(row, ("v2-zip".into(), 2, "legacy-plugin".into(), 0));
        let document_sources_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'document_sources'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(document_sources_count, 1);
    }
}
