...
pub unsafe extern "system" fn WaitOnAddress(
    ...
    let _ = AddressSize;
    let ms = if dwMilliseconds == INFINITE {
        1000
    } else {
        dwMilliseconds
    };
...
pub unsafe extern "system" fn GetSystemTimePreciseAsFileTime(lpSystemTimeAsFileTime: LPFILETIME) {
    unsafe {
        ...
        if !KERNEL32_DLL.is_null() {
            let f: Option<unsafe extern "system" fn(LPFILETIME)> = std::mem::transmute(
                GetProcAddress(KERNEL32_DLL, b"GetSystemTimePreciseAsFileTime\0".as_ptr()),
            );
            if let Some(f) = f {
