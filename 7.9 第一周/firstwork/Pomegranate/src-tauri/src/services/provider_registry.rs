...
impl AiProviderRegistry {
    ...
    pub fn unregister(&self, provider_type: &str) {
        ...
        if plugins.remove(provider_type).is_some() {
            log::info!("[ProviderRegistry] Unregistered provider: {}", provider_type);
        }
    }
    ...
    async fn chat_stream(
        ...
        let (api_url, model_id, api_key) = {
            let config = self.config.read().map_err(|e| {
                AppError::Custom(format!("Config lock error: {}", e))
            })?;
            (
        ...
        if !options.stream {
            ...
            let response = request.send().await.map_err(|e| {
                AppError::Custom(format!("API 请求失败: {}", e))
            })?;
            ...
            let data: Value = response.json().await.map_err(|e| {
                AppError::Custom(format!("JSON 解析失败: {}", e))
            })?;
    ...
    registry.register(AiProviderPlugin {
        ...
        factory: Box::new(|config: AiProviderConfig| {
            let metadata = AiProviderMetadata {
                ...
                description: format!(
                    "DeepSeek API — {} @ {}",
                    config.model_id, config.api_url
                ),
            ...
            Ok(Arc::new(OpenAiCompatibleProvider::new(metadata, config))
