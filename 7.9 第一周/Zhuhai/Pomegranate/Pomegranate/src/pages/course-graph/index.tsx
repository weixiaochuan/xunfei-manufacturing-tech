...
import type { CourseGraphConfig, CourseGraphHealth, CourseGraphStats } from "@/types";
...
export default function CourseGraphPage() {
  ...
  async function handleSelectNode(node: CourseGraphNode) {
    ...
    setNodeDetail(null);
    try {
      const detail = await courseGraphApi.nodeDetail(node.businessId);
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <Typography.Text code copyable>
                  {config?.databasePath ?? "Pomegranate/src-tauri/resources/course-graph/process_graph.db"}
                </Typography.Text>
              </div>
              <div>
                修复方式：确认课程图谱资源文件已放入
                <Typography.Text code>Pomegranate/src-tauri/resources/course-graph/process_graph.db</Typography.Text>
                ，并随 Tauri 打包配置一同发布。
              </div>
            </div>
          }
    ...
    ...graphEdges
      ...
      .map((edge) => ({
        ...
        data: {
          ...
          target: edge.target,
          label: edge.label,
          color: edge.label === "RELATED_TO" ? "#f97316" : "#94a3b8",
          width: edge.label === "RELATED_TO" ? 2.4 : 1.4,
        },
      })),
  ];
}
...
function layoutOptions(layout: string) {
  if (layout === "grid") {
    return { name: "grid", fit: true, padding: 36, avoidOverlap: true };
  }
  if (layout === "concentric") {
    return { name: "concentric", fit: true, padding: 36, minNodeSpacing: 36 };
  }
  return {
    name: "cose",
    fit: true,
