...
test("public-ip-test accepts only the approved temporary origins", () => {
  ...
  assert.equal(config.oidc.nbfClockToleranceSeconds, 0);
});
...
test("OIDC nbf clock tolerance is restricted to the explicit local TEST boundary", () => {
  configureBaseEnvironment();
  Object.assign(process.env, {
    DEPLOYMENT_PROFILE: "local",
    ALLOW_INSECURE_PUBLIC_IP_TEST: "false",
    ...
    CASDOOR_PUBLIC_URL: PUBLIC_IP_TEST_CASDOOR_ORIGIN,
    CASDOOR_REDIRECT_URI: "http://127.0.0.1:18080/auth/callback",
    CASDOOR_NBF_CLOCK_TOLERANCE_SECONDS: "28860",
  });

  assert.equal(loadConfig().oidc.nbfClockToleranceSeconds, 28_860);

  process.env.ALLOW_LOCAL_TEST_CASDOOR = "false";
  process.env.CASDOOR_PUBLIC_URL = "http://127.0.0.1:8000";
  assert.throws(() => loadConfig(), /CASDOOR_NBF_CLOCK_TOLERANCE_SECONDS/);

  process.env.ALLOW_LOCAL_TEST_CASDOOR = "true";
  process.env.CASDOOR_PUBLIC_URL = PUBLIC_IP_TEST_CASDOOR_ORIGIN;
  process.env.DEPLOYMENT_PROFILE = "public-ip-test";
  process.env.ALLOW_INSECURE_PUBLIC_IP_TEST = "true";
  process.env.ACCOUNT_SERVER_HOST = "0.0.0.0";
  process.env.ACCOUNT_SERVER_PORT = "3010";
  ...
  process.env.CASDOOR_NBF_CLOCK_TOLERANCE_SECONDS = "32401";
  assert.throws(() => loadConfig(), /0.*32400/);
});
