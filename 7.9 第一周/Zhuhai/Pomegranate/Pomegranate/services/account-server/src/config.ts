...
function readOidcNbfClockTolerance(
  profile: DeploymentProfile,
  allowLocalTestCasdoor: boolean,
  casdoorPublicUrl: string,
): number {
  const rawValue = (process.env.CASDOOR_NBF_CLOCK_TOLERANCE_SECONDS ?? "0").trim();
  if (!/^\d+$/.test(rawValue)) {
    throw new Error("CASDOOR_NBF_CLOCK_TOLERANCE_SECONDS 必须是非负整数");
  }
  const value = Number(rawValue);
  if (!Number.isSafeInteger(value) || value < 0 || value > 32_400) {
    throw new Error("CASDOOR_NBF_CLOCK_TOLERANCE_SECONDS 必须在 0 到 32400 之间");
  }
  if (
    value > 0 &&
    !(
      ...
      casdoorPublicUrl === PUBLIC_IP_TEST_CASDOOR_ORIGIN
    )
  ) {
    throw new Error("CASDOOR_NBF_CLOCK_TOLERANCE_SECONDS 只能用于明确启用的本地 Casdoor TEST");
  }
  return value;
}
...
export function loadConfig(): AccountServerConfig {
  ...
  const nbfClockToleranceSeconds = readOidcNbfClockTolerance(
    deploymentProfile,
    allowLocalTestCasdoor,
    casdoorPublicUrl,
  );
