...
use crate::account::{
    account_authorization_header, account_server_endpoint, atomic_replace, clear_account_credential,
    emit_account_signed_out, load_account_document_session, AccountState,
};
...
fn imported_markdown_title(path: &Path) -> String {
    ...
    if title.is_empty() { "未命名文档".to_string() } else { title.to_string() }
}
...
fn is_safe_office_type(name: &str, _mime_type: Option<&str>) -> bool {
    ...
    let executable = [
        "exe", "msi", "com", "scr", "bat", "cmd", "ps1", "vbs", "js", "jar", "reg",
        "lnk", "hta",
    ];
...
fn user_message(status: StatusCode, error: Option<&str>) -> AccountDocumentError {
    match status {
        StatusCode::UNAUTHORIZED => AccountDocumentError::new("signedOut", "登录已失效，请重新登录"),
        ...
        StatusCode::BAD_REQUEST if error == Some("file_type_mismatch") || error == Some("file_type_rejected") => {
...
pub async fn account_list_documents(
    ...
    if let Some(kind) = input.kind {
        if kind != "markdown" && kind != "uploaded_file" {
            return Err(AccountDocumentError::new("invalidFilter", "文档类型筛选无效"));
        }
    ...
    if body.status != "ok" {
        return Err(AccountDocumentError::new("invalidResponse", "账号文档服务返回无效数据"));
    }
    Ok(body.documents)
}
...
pub async fn account_create_markdown_document(
    ...
    let response = request(&app, &documents, &account, Method::POST, "/documents/markdown")
        .await?
        .json(&input)
        .send()
        .await
...
pub async fn account_import_markdown_file(
    ...
    let path = selected.into_path().map_err(|_| {
        AccountDocumentError::new("markdownReadFailed", "无法读取所选文件")
    })?;
...
pub async fn account_delete_document(
    ...
    validate_uuid(&document_id, "notFound")?;
    document_action(&app, &documents, &account, Method::DELETE, &format!("/documents/{document_id}"))
        .await?;
    Ok(())
}
...
pub async fn account_list_document_folders(
    ...
    let response = request(&app, &documents, &account, Method::GET, "/document-folders")
        ...
        .await
        .map_err(|_| AccountDocumentError::new("unavailable", "账号文档服务暂不可用"))?;
    Ok(parse_response::<FoldersResponse>(&app, &account, response).await?.folders)
}
...
pub async fn account_create_document_folder(
    ...
    let response = request(&app, &documents, &account, Method::POST, "/document-folders")
        .await?
        .json(&input)
        .send()
        .await
...
pub async fn account_update_document_folder(
    ...
    let response = request(&app, &documents, &account, Method::PATCH, &format!("/document-folders/{folder_id}"))
        .await?
        .json(&input)
        .send()
        .await
...
pub async fn account_delete_document_folder(
    ...
    validate_uuid(&folder_id, "notFound")?;
    document_action(&app, &documents, &account, Method::DELETE, &format!("/document-folders/{folder_id}"))
        .await?;
    Ok(())
}
...
pub async fn account_list_document_tags(
    ...
    let response = request(&app, &documents, &account, Method::GET, "/document-tags")
        ...
        .await
        .map_err(|_| AccountDocumentError::new("unavailable", "账号文档服务暂不可用"))?;
    Ok(parse_response::<TagsResponse>(&app, &account, response).await?.tags)
}
...
pub async fn account_create_document_tag(
    ...
    let response = request(&app, &documents, &account, Method::POST, "/document-tags")
        ...
        .await
        .map_err(|_| AccountDocumentError::new("unavailable", "账号文档服务暂不可用"))?;
    Ok(parse_response::<TagResponse>(&app, &account, response).await?.tag)
}
...
pub async fn account_update_document_tag(
    ...
    let response = request(&app, &documents, &account, Method::PATCH, &format!("/document-tags/{tag_id}"))
        .await?
        .json(&input)
        .send()
        .await
...
pub async fn account_delete_document_tag(
    ...
    validate_uuid(&tag_id, "notFound")?;
    document_action(&app, &documents, &account, Method::DELETE, &format!("/document-tags/{tag_id}"))
        .await?;
    Ok(())
}
...
fn cache_path(app: &AppHandle, platform_user_id: &str, file_id: &str, sha256: &str, name: &str) -> Result<PathBuf, AccountDocumentError> {
    let data_dir = app
        .path()
        .app_data_dir()
...
async fn ensure_cached_upload(
    ...
    let (token, user) = load_account_document_session(account)
        .await
        .map_err(|error| AccountDocumentError::new(error.code, error.message))?;
    let target = cache_path(app, &user.platform_user_id, file_id, &expected_sha, &safe_name)?;
    ...
    if !valid_cache {
        ...
        let response = documents
            .client
            .get(account_server_endpoint(&format!("/files/{file_id}/download")))
            .header(reqwest::header::AUTHORIZATION, authorization)
            .send()
            .await
            .map_err(|_| AccountDocumentError::new("unavailable", "账号文档服务暂不可用"))?;
        ...
        if hash_hex(&content) != expected_sha {
            return Err(AccountDocumentError::new("hashMismatch", "文件下载校验失败"));
        }
        ...
        if hash_hex(&written) != expected_sha {
            let _ = tokio::fs::remove_file(&target).await;
            return Err(AccountDocumentError::new("hashMismatch", "文件缓存校验失败"));
        }
    }
    Ok((target, safe_name))
}
...
fn is_editable_uploaded_type(name: &str) -> bool {
    matches!(
        Path::new(name).extension().and_then(|value| value.to_str()).unwrap_or_default().to_ascii_lowercase().as_str(),
...
fn workspace_paths(
    ...
    if workspace_id.len() != 64 || !workspace_id.bytes().all(|value| value.is_ascii_hexdigit()) {
        return Err(AccountDocumentError::new("workspaceInvalid", "编辑工作副本无效"));
    }
    let data_dir = app.path().app_data_dir()
        .map_err(|_| AccountDocumentError::new("workspaceFailed", "无法访问应用数据目录"))?;
    let directory = data_dir.join(WORKSPACE_DIR_NAME).join(hash_hex(platform_user_id.as_bytes())).join(workspace_id);
    Ok((directory.join(safe_file_name(original_name)), directory.join(WORKSPACE_METADATA_NAME)))
}
...
async fn read_workspace(
    ...
    let (_, user) = load_account_document_session(account).await
        .map_err(|error| AccountDocumentError::new(error.code, error.message))?;
    let data_dir = app.path().app_data_dir()
        .map_err(|_| AccountDocumentError::new("workspaceFailed", "无法访问应用数据目录"))?;
    let metadata_path = data_dir.join(WORKSPACE_DIR_NAME).join(hash_hex(user.platform_user_id.as_bytes())).join(workspace_id).join(WORKSPACE_METADATA_NAME);
    ...
    let (file_path, expected_metadata_path) = workspace_paths(app, &user.platform_user_id, workspace_id, &metadata.original_name)?;
    if expected_metadata_path != metadata_path {
        return Err(AccountDocumentError::new("workspaceInvalid", "编辑工作副本归属无效"));
    }
    Ok((metadata, file_path, metadata_path))
}

async fn write_workspace_metadata(path: &Path, metadata: &UploadedWorkspaceMetadata) -> Result<(), AccountDocumentError> {
...
pub async fn account_begin_uploaded_document_edit(
    ...
    if !is_editable_uploaded_type(&safe_name) {
        return Err(AccountDocumentError::new("fileTypeRejected", "此文件类型仅支持打开查看"));
    }
    let (_, user) = load_account_document_session(&account).await
        .map_err(|error| AccountDocumentError::new(error.code, error.message))?;
    let workspace_id = hash_hex(format!("{document_id}:{file_id}").as_bytes());
    let (workspace_file, metadata_path) = workspace_paths(&app, &user.platform_user_id, &workspace_id, &safe_name)?;
    if let (Ok(existing), Ok(bytes)) = (tokio::fs::read(&metadata_path).await, tokio::fs::read(&workspace_file).await) {
        if let Ok(metadata) = serde_json::from_slice::<UploadedWorkspaceMetadata>(&existing) {
            if metadata.document_id == document_id && metadata.file_id == file_id && hash_hex(&bytes) != metadata.baseline_sha256 {
                app.opener().open_path(workspace_file.to_string_lossy().into_owned(), None::<&str>)
                    .map_err(|_| AccountDocumentError::new("openFailed", "无法打开编辑工作副本"))?;
                return Ok(UploadedWorkspaceResult { status: "modified".into(), workspace_id });
            }
        }
    }
    let (cached, _) = ensure_cached_upload(&app, &documents, &account, &file_id, &safe_name, &baseline_sha256).await?;
    ...
    write_workspace_metadata(&metadata_path, &UploadedWorkspaceMetadata {
        document_id, file_id, original_name: safe_name, mime_type, baseline_sha256,
    }).await?;
    app.opener().open_path(workspace_file.to_string_lossy().into_owned(), None::<&str>)
        .map_err(|_| AccountDocumentError::new("openFailed", "无法打开编辑工作副本"))?;
    Ok(UploadedWorkspaceResult { status: "monitoring".into(), workspace_id })
}
...
pub async fn account_check_uploaded_document_edit(
    ...
    let bytes = tokio::fs::read(file_path).await
        .map_err(|_| AccountDocumentError::new("workspaceNotFound", "编辑工作副本不存在"))?;
    let status = if hash_hex(&bytes) == metadata.baseline_sha256 { "monitoring" } else { "modified" };
    Ok(UploadedWorkspaceResult { status: status.into(), workspace_id })
}
...
pub async fn account_sync_uploaded_document_edit(
    ...
    let (mut metadata, file_path, metadata_path) = read_workspace(&app, &account, &workspace_id).await?;
    let bytes = tokio::fs::read(&file_path).await
        .map_err(|_| AccountDocumentError::new("workspaceNotFound", "编辑工作副本不存在"))?;
    if hash_hex(&bytes) == metadata.baseline_sha256 {
        return Ok(UploadedSyncResult { status: "unchanged".into(), workspace_id, file: None, document_id: None, revision: None, updated_at: None });
    }
    let (token, _) = load_account_document_session(&account).await
        .map_err(|error| AccountDocumentError::new(error.code, error.message))?;
    let authorization = account_authorization_header(&token)
        .map_err(|error| AccountDocumentError::new(error.code, error.message))?;
    let mut file_part = reqwest::multipart::Part::bytes(bytes).file_name(metadata.original_name.clone());
    if let Some(mime) = &metadata.mime_type {
        file_part = file_part.mime_str(mime).map_err(|_| AccountDocumentError::new("fileTypeRejected", "文件类型信息无效"))?;
    }
    let form = reqwest::multipart::Form::new()
        .text("expectedSha256", metadata.baseline_sha256.clone())
        .part("file", file_part);
    let response = documents.client
        .put(account_server_endpoint(&format!("/files/{}/content", metadata.file_id)))
        .header(reqwest::header::AUTHORIZATION, authorization)
        .multipart(form).send().await
        .map_err(|_| AccountDocumentError::new("unavailable", "账号文档服务暂不可用"))?;
    let body: ReplaceFileResponse = parse_response(&app, &account, response).await?;
    if body.status != "ok" || body.document_id != metadata.document_id || body.file.id != metadata.file_id {
        return Err(AccountDocumentError::new("invalidResponse", "账号文档服务返回无效数据"));
    }
    metadata.baseline_sha256 = body.file.sha256.clone();
    write_workspace_metadata(&metadata_path, &metadata).await?;
    Ok(UploadedSyncResult { status: "synced".into(), workspace_id, file: Some(body.file), document_id: Some(body.document_id), revision: Some(body.revision), updated_at: Some(body.updated_at) })
}
...
pub async fn account_discard_uploaded_document_edit(
    ...
    let authorization = account_authorization_header(&token)
        .map_err(|error| AccountDocumentError::new(error.code, error.message))?;
    let response = documents.client
        .get(account_server_endpoint(&format!("/files/{}/download", metadata.file_id)))
        .header(reqwest::header::AUTHORIZATION, authorization).send().await
        .map_err(|_| AccountDocumentError::new("unavailable", "账号文档服务暂不可用"))?;
    if !response.status().is_success() {
        parse_response::<serde_json::Value>(&app, &account, response).await?;
        return Err(AccountDocumentError::new("downloadFailed", "服务端版本下载失败"));
    }
    let sha = response.headers().get("x-pomegranate-content-sha256")
        .and_then(|value| value.to_str().ok()).ok_or_else(|| AccountDocumentError::new("invalidResponse", "服务端文件校验信息缺失"))?;
    let expected = validate_sha256(sha)?;
    let bytes = response.bytes().await.map_err(|_| AccountDocumentError::new("downloadFailed", "服务端版本下载失败"))?;
    if hash_hex(&bytes) != expected {
        return Err(AccountDocumentError::new("hashMismatch", "服务端版本校验失败"));
    }
    write_cache_file(&file_path, &bytes).await?;
    metadata.baseline_sha256 = expected;
    write_workspace_metadata(&metadata_path, &metadata).await?;
    app.opener().open_path(file_path.to_string_lossy().into_owned(), None::<&str>)
        .map_err(|_| AccountDocumentError::new("openFailed", "无法打开编辑工作副本"))?;
    Ok(UploadedWorkspaceResult { status: "monitoring".into(), workspace_id })
}
...
fn extract_ooxml_text(path: &Path, extension: &str) -> Result<String, AccountDocumentError> {
    ...
    for name in names {
        let mut entry = archive
            .by_name(&name)
            .map_err(|_| AccountDocumentError::new("materialUnsupported", "该 Office 文件缺少可读取正文"))?;
        let mut xml = String::new();
        entry
            .read_to_string(&mut xml)
            .map_err(|_| AccountDocumentError::new("materialUnsupported", "该 Office 文件正文编码无法读取"))?;
        output.push_str(&xml_visible_text(&xml));
        output.push('\n');
    }
    if output.trim().is_empty() {
        return Err(AccountDocumentError::new("materialUnsupported", "该 Office 文件没有可提取正文"));
    }
    Ok(output)
}
...
pub async fn account_open_uploaded_document(
    ...
    if !allow_unknown && !is_safe_office_type(&safe_name, mime_type.as_deref()) {
        return Ok(OpenUploadedDocumentResult::ConfirmationRequired { file_name: safe_name });
    }
    let (target, _) = ensure_cached_upload(&app, &documents, &account, &file_id, &original_name, &sha256).await?;
...
pub async fn account_prepare_uploaded_document_material(
    ...
    let (target, safe_name) = ensure_cached_upload(&app, &documents, &account, &file_id, &original_name, &sha256).await?;
    let extension = Path::new(&safe_name)
        .extension()
        .and_then(|value| value.to_str())
        .unwrap_or_default()
    ...
    let parsed = tokio::task::spawn_blocking(move || {
        ...
        let preview = crate::services::ai::AiService::parse_attachment_auto(&path)
            .map_err(|_| AccountDocumentError::new("materialUnsupported", "该文件类型暂不能作为 PPT 文字素材"))?;
...
mod tests {
    ...
    fn cache_identity_changes_for_every_user_file_or_hash() {
        let key = |user: &str, file: &str, sha: &str| hash_hex(format!("{user}:{file}:{sha}").as_bytes());
    ...
    fn external_edit_allowlist_accepts_documents_and_rejects_executables() {
        for allowed in ["a.md", "a.txt", "a.docx", "a.xlsx", "a.pptx", "a.csv", "a.yaml"] {
            assert!(is_editable_uploaded_type(allowed), "{allowed}");
        }
        for rejected in ["a.exe", "a.msi", "a.dll", "a.bat", "a.cmd", "a.ps1", "a.js", "a.jar", "a.lnk", "a.pdf"] {
    ...
    fn workspace_identity_is_stable_per_document_and_file() {
        let identity = |document: &str, file: &str| hash_hex(format!("{document}:{file}").as_bytes());
    ...
    fn editable_markdown_import_accepts_only_md_and_markdown() {
        ...
        for rejected in ["note.mdx", "note.mdxl", "note.txt", "note.exe", "note"] {
            assert!(!is_editable_markdown_path(Path::new(rejected)), "{rejected}");
        }
    }
    ...
    fn editable_markdown_import_is_strict_utf8_and_strips_a_bom() {
        ...
        assert_eq!(
            decode_imported_markdown([&[0xef, 0xbb, 0xbf][..], "正文".as_bytes()].concat()).unwrap(),
            "正文",
        );
    ...
    fn markdown_mutation_omits_absent_optional_fields_from_http_json() {
        ...
        assert_eq!(json.get("title").and_then(|value| value.as_str()), Some("未命名文档"));
        assert_eq!(json.get("markdownContent").and_then(|value| value.as_str()), Some(""));
        for absent in [
            "expectedRevision",
            "folderId",
            "diaryDate",
            ...
            assert!(!json.as_object().unwrap().contains_key(absent), "{absent} must be omitted");
        }
    }
}
