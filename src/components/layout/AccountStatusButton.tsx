...
function relaunchForAccountScope(messageText: string): void {
  message.loading(messageText, 1);
  window.setTimeout(() => {
    void relaunch();
  }, 500);
}
...
export function AccountStatusButton() {
  ...
  useEffect(() => {
    ...
    const apply = (result: AccountLoginResult) => {
      ...
      if (result.status === "success" && result.restartRequired) {
        relaunchForAccountScope("正在切换账号专属数据空间，软件将自动重启");
        return;
      }
            ...
            if (key === "logout") {
              void logout().then((result) => {
                if (result.status === "signedOut") {
                  relaunchForAccountScope("正在退出账号专属数据空间，软件将自动重启");
