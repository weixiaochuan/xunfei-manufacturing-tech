...
const DeveloperCenterPage = lazy(() => import("@/pages/developer-center"));
const ReviewCenterPage = lazy(() => import("@/pages/review-center"));
      ...
      >
        <div className="flex h-full flex-col gap-3">
          ...
          <Space wrap size={[6, 6]}>
            ...
            {item.packageFormat === "v3-firstwork-plugin" && <Tag color="blue">Manifest v3</Tag>}
            {item.classification && <Tag color="cyan">{item.classification}</Tag>}
          ...
          <div className="mt-auto flex items-center justify-between pt-2">
            <Text type="secondary" className="text-xs">
              ...
              {item.supportedScenes.length > 0 ? ` · ${item.supportedScenes.join("/")}` : ""}
            </Text>
        ...
        {detailLoading || !detail ? (
          ...
          <div className="space-y-4">
            ...
            <Descriptions column={1} size="small" bordered>
              ...
              <Descriptions.Item label="插件包">{detail.packageFormat === "v3-firstwork-plugin" ? `Manifest v${detail.manifestSchemaVersion} / .firstwork-plugin` : "Manifest v2 / .zip 兼容"}</Descriptions.Item>
              <Descriptions.Item label="插件分类">{detail.classification ?? "legacy"}</Descriptions.Item>
              <Descriptions.Item label="适用场景">{detail.supportedScenes.join("、") || "未声明"}</Descriptions.Item>
            ...
            <Space wrap>
              ...
              {detail.installed && (
                <Button
                  type={detail.enabled && detail.classification !== "enhancement" ? "primary" : "default"}
                  onClick={() => navigate("/marketplace?section=plugins")}
                >
                  {detail.classification === "feature"
                    ? "打开功能"
                    : detail.classification === "hybrid"
                      ? "打开功能与增强设置"
                      : detail.classification === "enhancement"
                        ? "管理增强场景"
                        : "管理插件"}
                </Button>
              )}
...
type MarketplaceSectionKey = "catalog" | "plugins" | "publish" | "review";

function normalizeMarketplaceSection(value: string | null): MarketplaceSectionKey {
  if (value === "plugins" || value === "publish" || value === "review") {
    return value;
  }
  return "catalog";
}

function LazyMarketplaceSection({ children }: { children: ReactNode }) {
  return (
    <Suspense
      fallback={
...
export default function MarketplacePage() {
  ...
  const section = normalizeMarketplaceSection(searchParams.get("section") ?? searchParams.get("tab"));

  const handleSectionChange = (next: string) => {
    const normalized = normalizeMarketplaceSection(next);
  ...
  return (
    <div className="min-h-full px-4 py-3">
      <Alert
        ...
        type="info"
        showIcon
        message="AI 应用市场已整合插件上传、审核、安装和启用"
        description="当前为本地一体化演示入口：同一用户可以浏览应用、上传插件包、执行审核上架、安装启用和管理插件。后续接入云端服务器后，可把这里的本地流程替换为真实账号、云端存储和远程市场同步。"
      />
      <Tabs
        activeKey={section}
        ...
        items={[
          {
            key: "catalog",
            label: "发现与安装",
            children: <MarketplaceCatalogPage />,
          },
          {
            key: "plugins",
            label: "插件管理",
            children: <LazyMarketplaceSection><PluginsPage /></LazyMarketplaceSection>,
          },
          {
            key: "publish",
            label: "上传与发布",
            children: <LazyMarketplaceSection><DeveloperCenterPage /></LazyMarketplaceSection>,
          },
          {
            key: "review",
            label: "审核与上架",
            children: <LazyMarketplaceSection><ReviewCenterPage /></LazyMarketplaceSection>,
          },
        ]}
      />
    </div>
