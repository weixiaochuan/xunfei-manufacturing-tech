...
import { aiModelApi, noteApi, researchApi } from "@/lib/api";
import {
  analysisKnowledgeNoteContent,
  ensureResearchLibraryFolder,
  isSameResearchPaper,
  listResearchLibraryNotes,
  researchAnalysisNoteTitle,
  researchPaperMarker,
...
type ResearchSortMode = "frontier" | "year" | "citations";
...
function knowledgeNoteContent(
  ...
  return `${researchPaperMarker(paper)}
...
export default function ResearchAssistantPage() {
  ...
  const [minCitationsFilter, setMinCitationsFilter] = useState<number>();
  const [sortMode, setSortMode] = useState<ResearchSortMode>("frontier");
  ...
  const [analysisNoteId, setAnalysisNoteId] = useState<number | null>(null);
  ...
  const defaultModel = useMemo(() => models.find((model) => model.is_default) ?? null, [models]);
  const visiblePapers = useMemo(() => {
    if (!searchResult) return [];
    const filtered = searchResult.papers.filter((paper) => {
      if (sourceFilter && !paper.sources.some((source) => source.name === sourceFilter)) return false;
      if (workTypeFilter && paper.workType !== workTypeFilter) return false;
      if (minYearFilter !== undefined && paper.publicationYear < minYearFilter) return false;
      if (minCitationsFilter !== undefined && paper.citedByCount < minCitationsFilter) return false;
      return true;
    });
    return [...filtered].sort((left, right) => {
      if (sortMode === "year") return right.publicationYear - left.publicationYear || right.frontierScore - left.frontierScore;
      if (sortMode === "citations") return right.citedByCount - left.citedByCount || right.frontierScore - left.frontierScore;
      return right.frontierScore - left.frontierScore || right.publicationYear - left.publicationYear;
    });
  }, [minCitationsFilter, minYearFilter, searchResult, sortMode, sourceFilter, workTypeFilter]);
  const sourceOptions = useMemo(
    () => searchResult?.sources.map((source) => ({ label: source.name, value: source.name })) ?? [],
    [searchResult],
  );
  const workTypeOptions = useMemo(() => {
    if (!searchResult) return [];
    return [...new Set(searchResult.papers.map((paper) => paper.workType))]
      .filter(Boolean)
      .sort((left, right) => workTypeLabel(left).localeCompare(workTypeLabel(right), "zh-CN"))
      .map((value) => ({ label: workTypeLabel(value), value }));
  }, [searchResult]);
  ...
  async function refreshModelStatus() {
    ...
    try {
      const nextModels = await aiModelApi.list();
      setModels(nextModels);
      const nextDefault = nextModels.find((model) => model.is_default);
      if (nextDefault) {
        const result = await aiModelApi.test({
          name: nextDefault.name,
          provider: nextDefault.provider,
          protocol: nextDefault.protocol,
          api_key: nextDefault.api_key,
          api_url: nextDefault.api_url,
          model_id: nextDefault.model_id,
          credential_id: nextDefault.credential_id,
          max_context: nextDefault.max_context,
          supports_tools: nextDefault.supports_tools,
          supports_vision: nextDefault.supports_vision,
          max_output_tokens: nextDefault.max_output_tokens,
        });
      ...
      setModelTestResult({
        ok: false,
        latency_ms: 0,
        sample: `默认模型状态检测失败：${String(error)}`,
      });
    } finally {
  ...
  async function choosePapers() {
    ...
    if (paths.length > 5) {
      message.warning("每次最多分析 5 篇论文，已保留前 5 篇");
    }
  ...
  async function analyzeUploadedPapers() {
    if (paperPaths.length < 1) {
      message.warning("请至少选择 1 篇 PDF 论文");
      return;
    }
    ...
    try {
      ...
      setAnalysisNoteId(null);
      message.success(paperPaths.length === 1 ? "单篇论文分析完成" : "多论文分析完成");
    } catch (error) {
  ...
  async function handleSearch(values: ResearchFormValues) {
    ...
    try {
      ...
      setWorkTypeFilter(undefined);
      setMinYearFilter(undefined);
      setMinCitationsFilter(undefined);
      setSortMode("frontier");
  ...
  async function handleAddPaperToKnowledgeBase(paper: ResearchPaper) {
    ...
    try {
      const folderId = await ensureResearchLibraryFolder();
      const libraryNotes = await listResearchLibraryNotes(folderId);
      const existingNote = libraryNotes.find((note) => isSameResearchPaper(note, paper));
      if (existingNote) {
        setAddedNoteIds((current) => ({ ...current, [paper.id]: existingNote.id }));
        message.info("论文知识库中已存在这篇论文，未重复创建");
        return;
      }
  ...
  async function handleSaveAnalysisToKnowledgeBase() {
    if (!analysisResult) return;
    setSavingAnalysis(true);
    try {
      const folderId = await ensureResearchLibraryFolder();
      const note = await noteApi.create({
        title: researchAnalysisNoteTitle(analysisResult),
        content: analysisKnowledgeNoteContent(analysisResult, projectContext),
        folder_id: folderId,
      });
      setAnalysisNoteId(note.id);
      message.success("论文分析结果已保存到论文知识库");
    } catch (error) {
      message.error(`保存分析结果失败：${String(error)}`);
    } finally {
  ...
  return (
    <div className="mx-auto max-w-[1180px] pb-8">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <Space wrap>
          <Button icon={<Library size={16} />} onClick={() => navigate("/research-library")}>
            打开论文知识库
          </Button>
          <Button
            icon={<Wifi size={16} />}
      ...
      <Alert
        className="mb-4"
        type={defaultModel && modelTestResult?.ok ? "success" : defaultModel ? "warning" : "info"}
        showIcon
        icon={<Settings size={18} />}
        message={
          defaultModel
            ? `默认模型：${defaultModel.name}（${defaultModel.provider} / ${defaultModel.model_id}）`
            : "尚未配置默认模型"
        }
        description={
          defaultModel
            ? modelTestResult
              ? modelTestResult.ok
                ? `模型连通性正常，延迟 ${modelTestResult.latency_ms}ms。`
                : `默认模型当前不可用：${modelTestResult.sample ?? "请检查模型配置和网络。"}`
              : "可点击“检测默认模型”确认真实模型状态。"
            : "PDF 分析和 AI 入库建议需要默认模型；模型不可用时，论文入库建议会显示本地规则 fallback。"
        }
      />
      ...
      <Card
        ...
        extra={<Tag color={defaultModel && modelTestResult?.ok ? "purple" : "orange"}>使用当前默认模型</Tag>}
      >
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={9}>
      ...
      {analysisResult ? (
        <Card
          className="mb-4"
          title={<Space><Save size={18} />论文分析结果</Space>}
          extra={
            <Space>
              {analysisNoteId ? (
                <Button type="link" onClick={() => navigate(`/notes/${analysisNoteId}`)}>
                  打开已保存笔记
                </Button>
              ) : null}
              <Button
                ...
                onClick={() => void handleSaveAnalysisToKnowledgeBase()}
              >
                保存到论文知识库
              </Button>
            </Space>
        ...
        >
          <ResearchAnalysisView result={analysisResult} />
        </Card>
      ) : null}
      ...
      {searchResult ? (
        <Space direction="vertical" size={16} style={{ width: "100%" }}>
              ...
              items={[
                {
                  ...
                  label: (
                    <Space>
                      ...
                      结果筛选与排序
                      <Tag>{visiblePapers.length}/{searchResult.papers.length}</Tag>
                    </Space>
                  ),
                  children: (
                    <Row gutter={[12, 12]} align="middle">
                      ...
                      <Col xs={12} md={4} lg={4}>
                        <InputNumber
                          style={{ width: "100%" }}
                          min={searchResult.fromYear}
                          max={searchResult.toYear}
                          placeholder="最低年份"
                          value={minYearFilter}
                          onChange={(value) => setMinYearFilter(value ?? undefined)}
                        />
                      </Col>
                      <Col xs={12} md={4} lg={4}>
                        <InputNumber
                          ...
                          onChange={(value) => setMinCitationsFilter(value ?? undefined)}
                        />
                      </Col>
                      <Col xs={24} lg={6}>
                        <Select
                          ...
                          value={sortMode}
                          options={[
                            { label: "按前沿分排序", value: "frontier" },
                            { label: "按发表年份排序", value: "year" },
                            { label: "按引用量排序", value: "citations" },
                          ]}
          ...
          {visiblePapers.length === 0 ? (
            <Card>
              <Empty description={searchResult.papers.length === 0 ? "没有找到匹配论文，请缩短主题或换一组关键词再试" : "没有符合当前筛选条件的论文"} />
            </Card>
          ) : (
            <Card
              title="跨平台近五年前沿论文"
                        ...
                        <Alert
                          ...
                          description={
                            <Space direction="vertical" size={4}>
                              <Text type="secondary">
                                论文题录、摘要、阅读提示、筛选依据和 AI 建议已保存为知识库笔记，可参与后续检索。
                              </Text>
                              <Button
                                type="link"
                                style={{ padding: 0 }}
                                onClick={() => navigate(`/notes/${addedNoteIds[paper.id]}`)}
                              >
                                打开论文笔记
                              </Button>
                            </Space>
